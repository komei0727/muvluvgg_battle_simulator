...
**branch protection の必須checkは `pr.yml` の集約 job `Quality Gate Result` だけにする。** reusable workflow 呼び出し（`quality` / `ui`）は skip されると check-run 名が caller job id だけ（`quality`）になり、実行時の名前（`quality / Quality Gate`）が生成されない — これらを直接必須checkに指定すると、変更分類がその job を skip した PR は必須 check が永久に生成されず `BLOCKED` のまま残る。集約 job は `if: always()` で必ず check-run を作り、変更分類（`run_api` / `run_ui`）に対して各 gate の `result` が期待どおりかを検証する（必須の gate が skip されていれば失敗させる）。判定ロジックは `scripts/ci/evaluate-quality-gate.mjs` にあり `mise run ci:test` でテストできる。
