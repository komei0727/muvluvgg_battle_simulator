...
describe("Catalog v2 production candidate: 10-unit promotion (Issue #46)", () => {
  it("IT-CAT-PROD-001: loads all 10 units from catalog/ without an integrity violation", () => {
    ...
    // `2026-09-03.3` は `UNIT_LAYLA_NURSE`（【商売上手のエンジェルナース】
    // レイラ・ジェンキンス）の新規投入。既存キャラクター`CHAR_LAYLA_JENKINS`
    // （既存バリアント`UNIT_LAYLA_ENTREPRENEUR`）の新衣装違い。Linked Effect
    // Group（`linkedEffectGroupRole`）・`timeLimit.owner: EFFECT_SOURCE`・
    // `APPLY_HEALING_LINK`・`removeOnSourceDefeated`・`damageThreshold`付き
    ...
    // 同スキル構成で、威力・閾値の数値も変更なし（`UNIT_LAYLA_NURSE_TEX`と同じ
    // パターン）。ただしPS2「いっしょにがんばる+」はプレイアブル版が持つ1戦闘1回
    // 制限（`RUNTIME_COUNTER`）を演習版では撤廃し、毎ターン開始時に発動する
    // （ユーザー確認済み）。差分はLv200固定のステータス（`category:
    // EXERCISE_ENEMY`）とこのPS2の回数制限撤廃のみ。エンジン拡張は伴わない。
