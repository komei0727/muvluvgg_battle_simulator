...
const unitBrokenDetailsSchema = {
  ...
  properties: {
    ...
    // ブレイク履歴はpayloadだけからの投影として定義される）。メモリー由来の
    // ブレイクは発生源ユニットを持たないため、どちらも必須にしない（R-MEM-04）。
    sourceUnitId: { type: "string" },
    sourceSide: { type: "string", enum: ["ALLY", "ENEMY"] },
  },
} as const;
...
/**
 * `ChargeReleaseCompleted`（R-SKL-05）。チャージ効果のEffectSequenceとPS/Memory連鎖が
 * 解決し終わった後に発行する。AS/EX経路の `SKILL_USE_COMPLETED` に相当し、
 * `skillType`/`resolvedStepCount`/`targetUnitIds` も同じ意味で持つ。
 */
const chargeReleaseCompletedDetailsSchema = {
  type: "object",
  additionalProperties: false,
  required: [
    "actorUnitId",
