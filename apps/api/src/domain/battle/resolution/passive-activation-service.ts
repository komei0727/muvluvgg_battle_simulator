...
import { matchesMemoryTrigger } from "../triggering/memory-trigger-matcher.js";
...
import {
  beginEffectSequenceResolution as beginEffectSequenceResolutionService,
  finalizeEffectSequenceResolution as finalizeEffectSequenceResolutionService,
  finalizeEffectSequenceResolutionSteps as finalizeEffectSequenceResolutionStepsService,
  finalizeResolutionScope as finalizeResolutionScopeService,
  type ResolutionScopeLifecycleContext,
} from "./resolution-scope-lifecycle-service.js";
import {
  buildPassiveChainDependencies,
  type PassiveChainDependencySource,
} from "./passive-chain-dependency-assembly.js";
...
export class PassiveActivationRuntime {
   ...
   * `counterUpdates`（あれば）を登録する。実装・詳細な理由は
   * `./resolution-scope-lifecycle-service.js`（REF-063 #3）へ抽出した。
   */
  beginEffectSequenceResolution(
    ...
    beginEffectSequenceResolutionService(
  ...
  /**
   * `resolvePassiveChain`/`resolvePendingCandidateGroups`へ注入する
   * `PassiveChainDependencies`の組み立て。実装・詳細な理由は
   * `./passive-chain-dependency-assembly.js`（REF-063 #3）へ抽出した。
   * `activatePassiveCandidate`/`reconfirmMemoryCandidate`/`activateMemoryCandidate`
   * （PS/Memory発動連鎖そのもの）はこのクラス側の責務のまま残る。
   */
  private buildDependencies(): PassiveChainDependencies {
    const source: PassiveChainDependencySource = {
      getUnits: () => this.units,
      getGuard: () => this.guard,
      ...
      ...(this.context.resolutionPhase !== undefined
        ...
        : {}),
      limits: this.limits,
      activatePassiveCandidate: (candidate, event) =>
      ...
      activateMemoryCandidate: (candidate, event) => this.activateMemoryCandidate(candidate, event),
      ...
      applyEffectSequenceRuntimeCounterUpdates: (event, resolveChild) =>
        this.applyEffectSequenceRuntimeCounterUpdates(event, resolveChild),
      toTriggerEvent: (event) => this.toTriggerEvent(event),
    };
    return buildPassiveChainDependencies(source);
  }
  ...
  /**
   * `beginEffectSequenceResolution`/`finalizeEffectSequenceResolution(Steps)`/
   * `finalizeResolutionScope`（`./resolution-scope-lifecycle-service.js`、
   * REF-063 #3）へ渡す共通envelope。
   */
  private resolutionScopeLifecycleContext(): ResolutionScopeLifecycleContext {
    return {
      recorder: this.context.recorder,
      turnNumber: this.context.turnNumber,
      cycleNumber: this.context.cycleNumber,
   ...
   * 完了した時点で必ずそのcounterを破棄する。実装・詳細な理由は
   * `./resolution-scope-lifecycle-service.js`（REF-063 #3）へ抽出した。
   */
  private *finalizeEffectSequenceResolutionSteps(
    skillUseId: SkillUseId,
  ): Generator<BattleDomainEvent, void, void> {
    yield* finalizeEffectSequenceResolutionStepsService(
      this.resolutionScopeLifecycleContext(),
      this.activeEffectSequenceResolutions,
      () => this.units,
      (units) => {
        this.units = units;
   ...
   * （`applyEffectActionGroups`の戻り）を受け取った直後に必ず1回呼ぶ。実装・
   * 詳細な理由は`./resolution-scope-lifecycle-service.js`（REF-063 #3）へ抽出した
   * （PS連鎖内部からはこのメソッドを呼んではならない、代わりに
  ...
  finalizeEffectSequenceResolution(skillUseId: SkillUseId): readonly BattleUnit[] {
    return finalizeEffectSequenceResolutionService(
      this.resolutionScopeLifecycleContext(),
      this.activeEffectSequenceResolutions,
      () => this.units,
      (units) => {
      ...
      skillUseId,
      (event, units) => this.onFactEvent(event, units).units,
    );
  }
  ...
  /**
   * `R-EFF-11`「解決スコープ終了時にリセットするcounter」。実装・詳細な理由は
   * `./resolution-scope-lifecycle-service.js`（REF-063 #3）へ抽出した。
   */
  finalizeResolutionScope(cursor: DomainEventId, units?: readonly BattleUnit[]): ResolutionResult {
    ...
    const result = finalizeResolutionScopeService(
      this.resolutionScopeLifecycleContext(),
      this.context.definitions.unitDefinitions,
      this.context.definitions.skillDefinitions,
      this.units,
      cursor,
      (event, currentUnits) => this.onFactEvent(event, currentUnits),
    );
    this.units = result.units;
    return result;
  }
