...
/**
 * R-NUM-01／R-NUM-02の数値契約に従い、原基準値（Battleが保持するbinary64値）が表す
 * 厳密な有理数へ整数のパーセントポイントを合成し、0方向へ切り捨てる。
 *
 * 浮動小数の倍率を先に作って掛けてはならない（R-NUM-02）— 数学上ちょうど整数になる積が
 * 整数のわずか下へずれ、切り捨てで1小さくなる（`45 × 1.4 = 62.99999999999999`）。
 *
 * 誤差を吸収して整数へ引き上げることもしない（R-NUM-02の帰結の項）。実際の端数は
 * 整数から数ULP下にある場合があり（`200057283 × 110.18% × 105% × 385%` は4 ULP下）、
 * 演算誤差（`129.2 × 250%` は1 ULP下）と区別できないためである。
 */
