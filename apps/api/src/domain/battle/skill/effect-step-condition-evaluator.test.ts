...
describe("evaluateEffectStepCondition", () => {
  ...
  describe("TARGET_SET_COUNT（CAP_EFFECT_STEP_SET_CONDITION、Issue #227 RES-004集合条件）", () => {
    ...
    it("UT-R-SKL-06-042（PRレビュー[P2]再指摘）: `wholeSet: true` evaluates a step-target-referencing TARGET_STATE against the whole resolved candidate set (some(), not just `current`)", () => {
      const alive = unit("ENEMY_A", "UNIT_A");
      const dead = unit("ENEMY_B", "UNIT_A", { currentHp: 0 });
      const condition: ConditionDefinition = {
        kind: "TARGET_STATE",
        target: STEP_TARGET,
        field: "IS_ALIVE",
      ...
      const ctx: EffectStepTargetContext = {
        stepTarget: STEP_TARGET,
        current: dead,
        resolveOtherReference: () => [dead, alive],
        unitDefinitions: new Map(),
        wholeSet: true,
      };
      // `current` alone (`dead`) would fail IS_ALIVE, but `wholeSet` ignores
      // `current` and checks the whole resolved set — `alive` satisfies it.
      expect(evaluateEffectStepCondition(condition, undefined, ctx)).toBe(true);
    });

    it("UT-R-SKL-06-043（PRレビュー[P2]再指摘）: `wholeSet: true` combines the step-target TARGET_STATE (some()) with TARGET_SET_COUNT into a single step-wide boolean", () => {
      const alive = unit("ENEMY_A", "UNIT_A");
      const condition: ConditionDefinition = {
        kind: "AND",
        conditions: [
      ...
      const ctxWithEmptyOther: EffectStepTargetContext = {
        stepTarget: STEP_TARGET,
        current: alive,
        resolveOtherReference: (reference) =>
          reference.kind === STEP_TARGET.kind &&
          reference.targetBindingId === STEP_TARGET.targetBindingId
            ? [alive]
            : [],
        unitDefinitions: new Map(),
        wholeSet: true,
      ...
      expect(
        evaluateEffectStepCondition(
          ...
          undefined,
          ctxWithEmptyOther,
          ctxWithEmptyOther.resolveOtherReference,
        ),
      ).toBe(false);

      const ctxWithNonEmptyOther: EffectStepTargetContext = {
        ...ctxWithEmptyOther,
        resolveOtherReference: () => [alive],
      };
      expect(
        evaluateEffectStepCondition(
          condition,
          undefined,
          ctxWithNonEmptyOther,
          ctxWithNonEmptyOther.resolveOtherReference,
        ),
      ).toBe(true);
    });
  });
