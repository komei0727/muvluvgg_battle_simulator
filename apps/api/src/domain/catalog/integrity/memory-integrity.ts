...
function validateMemoryTriggerGrantedBy(
  ...
  violations.push({
    ...
    message:
      "TARGET_HAS_EFFECT/TARGET_EFFECT_COUNT's grantedBy cannot be used in a Memory trigger condition: Memory has no owning BattleUnit (R-MEM-04), so the evaluator receives only ownerSide and the AppliedEffect.sourceUnitId comparison would silently never match (DMG-007 Issue #187, TARGET_EFFECT_COUNT added by Issue #649)",
  });
}
