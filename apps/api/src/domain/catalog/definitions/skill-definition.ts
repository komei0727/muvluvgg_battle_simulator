...
function createResolution(
  ...
  if (input.kind === "CHARGE") {
    ...
    if (sequence.steps.length > 0) {
      throw new DomainValidationError(
        `${path}.steps`,
        "is not supported on a CHARGE skill's own top-level EffectSequence (resolveChargeStart never resolves it; declare the EffectActions on chargeRelease instead)",
      );
    }
