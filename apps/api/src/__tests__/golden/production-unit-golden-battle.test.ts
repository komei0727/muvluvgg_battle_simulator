 ...
 * 決定的に完走し、基本不変条件を満たすことを検証する。公開イベントの要約（種別ごとの
 * 件数＋順序ハッシュ、`summarizeEventSequence`）と勝敗を小さく安定したsnapshotで固定し、
 * ユニット追加に伴うルール間結合の回帰を検出する。
 *
...
describe("production Unit golden battles", () => {
  ...
  it.each(PRODUCTION_UNIT_IDS)(
    ...
    (unitDefinitionId) => {
      ...
      // 小さく安定したsnapshot: 勝敗・終了理由・公開イベントの要約のみ
