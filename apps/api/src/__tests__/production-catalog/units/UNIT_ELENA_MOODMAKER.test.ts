...
describe("production Catalog UNIT_ELENA_MOODMAKER (【心色見つめるムードメーカー】エレーナ・パステルコワ)", () => {
  ...
  it("IT-UNIT-ELENA-MOODMAKER-015 (R-CRT-04): AS2の実「対象の現在HP×12.5%分のダメージ」は会心判定を行わない — 会心率100%かつ会心保証を保持していても非会心で、会心の抽選も消費しない", () => {
    const observed = observeHitPointRatioCritical({
      ...
      effectActionDefinitionId: "ACT_ELENA_MOODMAKER_AS2_DAMAGE",
      skillDefinitionId: "SKL_ELENA_MOODMAKER_AS2",
      battleId: "B_ELENA_CRT04",
    });

    expect(observed.criticalMode).toBe("PREVENTED");
    expect(observed.isCritical).toBe(false);
    ...
    const control = observeHitPointRatioCritical({
      ...
      effectActionDefinitionId: "ACT_ELENA_MOODMAKER_PS2_DAMAGE",
      skillDefinitionId: "SKL_ELENA_MOODMAKER_PS2",
      battleId: "B_ELENA_CRT04_CONTROL",
    });

    expect(control.criticalMode).toBe("GUARANTEED");
  ...
  it("IT-UNIT-ELENA-MOODMAKER-016 (R-CRT-04 boundary): 会心保証を持たない使用者でもAS2は会心判定へ進まない — 会心率100%の同じ盤面で、PS2のDAMAGEより抽選を1本少なく消費する", () => {
    const probe = (effectActionDefinitionId: string, skillDefinitionId: string) =>
      observeHitPointRatioCritical({
        ...
        battleId: `B_ELENA_CRT04_${skillDefinitionId}`,
      });

    const as2 = probe("ACT_ELENA_MOODMAKER_AS2_DAMAGE", "SKL_ELENA_MOODMAKER_AS2");
    const ps2 = probe("ACT_ELENA_MOODMAKER_PS2_DAMAGE", "SKL_ELENA_MOODMAKER_PS2");

    expect(as2.criticalMode).toBe("PREVENTED");
    expect(as2.isCritical).toBe(false);
    expect(ps2.criticalMode).toBe("NORMAL");
    expect(ps2.isCritical).toBe(true);
    expect(ps2.randomDraws - as2.randomDraws).toBe(1);
  });
});
