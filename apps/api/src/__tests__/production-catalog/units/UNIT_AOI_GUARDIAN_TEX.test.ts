...
/**
 * `UNIT_AOI_GUARDIAN_TEX`（【厳格な規律の守護者】生駒葵・戦術演習版）のユニット単位
 ...
 * 唯一の接点になる。プレイアブル版 `UNIT_AOI_GUARDIAN` との差分は、ステータスと
 * EX回復3%（60%→）・PS1回復 失HPの5%（50%→）・PS2反撃 威力110（被ダメ100%→）・
 * スキル名「捲土重来+」の4点である。
 */

const UNIT_DEFINITION_ID = "UNIT_AOI_GUARDIAN_TEX";
...
const AS1_SHIELD = "ACT_AOI_GUARDIAN_TEX_AS1_SHIELD";
...
const BEHAVIOURS: readonly SkillBehaviourCase[] = [
  {
    skillDefinitionId: "SKL_AOI_GUARDIAN_TEX_EX",
    intent:
      "自身のHPを最大HPの3%回復し、2行動の間攻撃力を50%上昇させ（重複可）、気絶無効を付与する。さらに自身に攻撃力×150%までのダメージを防ぐシールドを付与する",
    use: { kind: "ACTIVE", skillDefinitionId: "SKL_AOI_GUARDIAN_TEX_EX" },
    expected: {
      actions: [
        {
          effectActionDefinitionId: "ACT_AOI_GUARDIAN_TEX_EX_HEAL",
          targets: ["ally:subject"],
        },
        {
          effectActionDefinitionId: "ACT_AOI_GUARDIAN_TEX_EX_ATK_UP",
          targets: ["ally:subject"],
        },
        {
          effectActionDefinitionId: "ACT_AOI_GUARDIAN_TEX_EX_STUN_IMMUNITY",
          targets: ["ally:subject"],
        },
        {
          effectActionDefinitionId: "ACT_AOI_GUARDIAN_TEX_EX_SHIELD",
          targets: ["ally:subject"],
        },
      ],
      // 最大HP10000の3%＝300（プレイアブル版の60%＝5000上限との差分点）。
      hpDeltas: {
        "ally:subject": 300,
      },
      effectsApplied: [
  ...
  {
    skillDefinitionId: "SKL_AOI_GUARDIAN_TEX_AS1",
    intent:
      "最もHP割合の低い味方単体に、攻撃力×120%までのダメージを防ぐシールドを付与する。さらに2行動の間、行動時に最大HPの5%を継続回復するバフを付与する",
    use: { kind: "ACTIVE", skillDefinitionId: "SKL_AOI_GUARDIAN_TEX_AS1" },
    board: {
      allies: [
        {
          id: "ally:front",
    ...
    expected: {
      ...
      actions: [
        {
          effectActionDefinitionId: "ACT_AOI_GUARDIAN_TEX_PS1_HEAL",
          targets: ["ally:subject"],
        },
        {
          effectActionDefinitionId: "ACT_AOI_GUARDIAN_TEX_AS1_SHIELD",
          targets: ["ally:front"],
        },
        {
          effectActionDefinitionId: "ACT_AOI_GUARDIAN_TEX_AS1_CONTINUOUS_HEAL",
          targets: ["ally:front"],
        },
      ],
      // 失ったHP 5000 の5%＝250（プレイアブル版の50%＝2500との差分点）。
      hpDeltas: {
        "ally:subject": 250,
      },
      effectsApplied: [
      ...
      cooldowns: [
        {
          unitId: "ally:subject",
          skillDefinitionId: "SKL_AOI_GUARDIAN_TEX_PS1",
          remaining: 1,
        },
        {
          unitId: "ally:subject",
  ...
  },
  {
    ...
    expected: {
      ...
      // 失ったHP 9000 の5%＝450。回復後もHP割合は最低のまま（AS1の対象は変わらない）。
      hpDeltas: {
        "ally:subject": 450,
      },
      effectsApplied: [
  ...
  {
    skillDefinitionId: "SKL_AOI_GUARDIAN_TEX_AS2",
    intent: "敵単体に威力212で攻撃する",
    use: { kind: "ACTIVE", skillDefinitionId: "SKL_AOI_GUARDIAN_TEX_AS2" },
    expected: {
      actions: [
        {
          effectActionDefinitionId: "ACT_AOI_GUARDIAN_TEX_PS1_HEAL",
        ...
        {
          effectActionDefinitionId: "ACT_AOI_GUARDIAN_TEX_AS2_DAMAGE",
          targets: ["enemy:front"],
        },
      ],
      hpDeltas: {
  ...
  {
    skillDefinitionId: "SKL_AOI_GUARDIAN_TEX_PS1",
    intent: "自身がアクティブスキルを使用する直前に発動。自身の失ったHPの5%を回復する",
    use: {
      kind: "PASSIVE",
      skillDefinitionId: "SKL_AOI_GUARDIAN_TEX_PS1",
  ...
  {
    skillDefinitionId: "SKL_AOI_GUARDIAN_TEX_PS2",
    intent:
      "自身がアクティブスキルで攻撃された後に発動。攻撃してきた敵単体に対して威力110で反撃をし、1行動分の気絶を付与する",
    use: {
      kind: "PASSIVE",
      skillDefinitionId: "SKL_AOI_GUARDIAN_TEX_PS2",
    ...
    expected: {
      actions: [
        {
          effectActionDefinitionId: "ACT_AOI_GUARDIAN_TEX_PS2_COUNTER",
          targets: ["enemy:front"],
        },
        {
          effectActionDefinitionId: "ACT_AOI_GUARDIAN_TEX_PS2_STUN",
          targets: ["enemy:front"],
        },
      ],
      // 威力110の反撃＝(攻撃力1000−防御力500)×1.10＝550（プレイアブル版の
      // 「受けたダメージの100%」＝500との差分点）。
      hpDeltas: {
        "enemy:front": -550,
      },
      effectsApplied: [
  ...
  },
  {
    ...
    expected: {
      actions: [
        ...
        {
          effectActionDefinitionId: "ACT_AOI_GUARDIAN_TEX_PS2_PP_ZERO",
          targets: ["enemy:front"],
        },
      ],
      // 3400 − 550 = 2850 ≦ 最大HP10000の30%（3000）でPP全削りの分岐へ入る。
      hpDeltas: {
        "enemy:front": -550,
      },
      effectsApplied: [
...
describe("production Catalog UNIT_AOI_GUARDIAN_TEX (【厳格な規律の守護者】生駒葵・戦術演習版)", () => {
  it.each(BEHAVIOURS)(
    "IT-UNIT-AOI-GUARDIAN-TEX-001: $skillDefinitionId — $intent",
  ...
  it("IT-UNIT-AOI-GUARDIAN-TEX-002: the table covers exactly the Skills the production UnitDefinition declares", () => {
  ...
  it("IT-UNIT-AOI-GUARDIAN-TEX-003: every EffectAction reachable from this unit was actually executed by the table above", () => {
    resetExecutedActionIds();
  ...
  it("IT-UNIT-AOI-GUARDIAN-TEX-004 (R-EFF-03): EXが配る「気絶無効」は `STATUS` カテゴリ全体ではなく気絶だけを拒否する。実 `ACT_LILY_HERO_AS2_STUN` は弾かれて同じ行動のstat debuffは通り、実 `ACT_NANAE_COMMANDER_EX_FREEZE` は同じ免疫を素通りする", () => {
    ...
    const guarded = applyPrecedingActions(board, [
      { effectActionDefinitionId: "ACT_AOI_GUARDIAN_TEX_EX_STUN_IMMUNITY", target: "SELF" },
    ]);
    const screen = (effectActionDefinitionIds: readonly string[], battleId: string) => {
      const { applied, rejected, immunity } = observeEffectImmunity({
        ...
        immunityEffectActionDefinitionId: "ACT_AOI_GUARDIAN_TEX_EX_STUN_IMMUNITY",
        battleId,
      });
    ...
    expect(
      screen(["ACT_LILY_HERO_AS2_STUN", "ACT_LILY_HERO_AS2_SPEED_DOWN"], "B_AOI_TEX_STUN"),
    ).toEqual({
      ...
      rejected: [
        {
          ...
          blockedBy: "ACT_AOI_GUARDIAN_TEX_EX_STUN_IMMUNITY",
        },
      ],
    ...
    // 免疫との区別がつかない。
    expect(screen(["ACT_NANAE_COMMANDER_EX_FREEZE"], "B_AOI_TEX_FREEZE")).toEqual({
  ...
  it("IT-UNIT-AOI-GUARDIAN-TEX-005 (R-SHD-02/R-SHD-03): AS1が配る実シールドはタイプなしプールとして被ダメージを吸収し、プールを超えた分だけがHPへ抜ける。1ヒットの振り分け（シールド吸収・HPダメージ・超過破棄）の合計は常に計算ダメージと一致する", () => {
    ...
    const absorbed = strike(shielded, 1, "B_AOI_TEX_SHIELD_ABSORB");
    ...
    const overflowed = strike(absorbed.units, 3, "B_AOI_TEX_SHIELD_OVERFLOW");
