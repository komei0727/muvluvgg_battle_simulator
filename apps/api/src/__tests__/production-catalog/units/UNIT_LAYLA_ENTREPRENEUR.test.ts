...
describe("production Catalog UNIT_LAYLA_ENTREPRENEUR (【戦うアントレプレナー】レイラ・ジェンキンス)", () => {
  ...
  it("IT-UNIT-LAYLA-ENTREPRENEUR-100: every declared Skill activates within one completed battle, with these counts and in this order", () => {
    ...
    expect(observation.activationCounts).toEqual({
      SKL_LAYLA_ENTREPRENEUR_AS1: 1,
      SKL_LAYLA_ENTREPRENEUR_AS2: 2,
      SKL_LAYLA_ENTREPRENEUR_PS1: 1,
      SKL_LAYLA_ENTREPRENEUR_PS2: 3,
      SKL_LAYLA_ENTREPRENEUR_EX: 1,
    });
    // PS1はターン開始時に1度だけ、PS2は会心4回ごと、EXはゲージ最大で予約発動する。
