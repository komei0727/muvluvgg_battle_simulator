...
/**
 * `UNIT_LAYLA_NURSE`（【商売上手のエンジェルナース】レイラ・ジェンキンス）のユニット単位
...
const UNIT_DEFINITION_ID = "UNIT_LAYLA_NURSE";

const snapshot = loadProductionSnapshot(PRODUCTION_CATALOG_DIR, [UNIT_DEFINITION_ID]);

const KANSATSU = "MARKER_LAYLA_NURSE_KANSATSU";

/** (SKL_ID, 原文の該当句, 前提盤面, 期待する振る舞い)。 */
const BEHAVIOURS: readonly SkillBehaviourCase[] = [
  {
    skillDefinitionId: "SKL_LAYLA_NURSE_EX",
    intent:
      "最もHP割合の低い敵を対象とし、対象が含まれる敵前後列に威力78で攻撃する。さらに対象が含まれる敵横一列に威力53で攻撃する",
    use: { kind: "ACTIVE", skillDefinitionId: "SKL_LAYLA_NURSE_EX" },
    board: {
      enemies: [
        {
          id: "enemy:front",
    ...
    expected: {
      ...
      actions: [
        {
          effectActionDefinitionId: "ACT_LAYLA_NURSE_EX_DAMAGE_COLUMN",
          targets: ["enemy:front"],
        },
        { effectActionDefinitionId: "ACT_LAYLA_NURSE_EX_DAMAGE_COLUMN", targets: ["enemy:back"] },
        { effectActionDefinitionId: "ACT_LAYLA_NURSE_EX_DAMAGE_ROW", targets: ["enemy:front"] },
        { effectActionDefinitionId: "ACT_LAYLA_NURSE_EX_DAMAGE_ROW", targets: ["enemy:left"] },
      ],
  ...
  {
    skillDefinitionId: "SKL_LAYLA_NURSE_AS1",
    intent: "「観察」状態の敵を優先し、敵単体に威力127.2で攻撃する",
    use: { kind: "ACTIVE", skillDefinitionId: "SKL_LAYLA_NURSE_AS1" },
    expected: {
      // 「観察」の保持者がいないため通常の対象選択(DEFAULT)へフォールバックする。
      actions: [
        { effectActionDefinitionId: "ACT_LAYLA_NURSE_AS1_DAMAGE", targets: ["enemy:front"] },
      ],
      // (1000-500)×1.272=636。
  ...
  },
  {
    ...
    expected: {
      ...
      actions: [
        { effectActionDefinitionId: "ACT_LAYLA_NURSE_AS1_DAMAGE_PIERCE", targets: ["enemy:left"] },
      ],
  ...
  {
    skillDefinitionId: "SKL_LAYLA_NURSE_PS1",
    intent:
    ...
    expected: {
      actions: [
        { effectActionDefinitionId: "ACT_LAYLA_NURSE_PS1_KANSATSU_MARK", targets: ["enemy:front"] },
        { effectActionDefinitionId: "ACT_LAYLA_NURSE_PS1_ATK_DOWN", targets: ["enemy:front"] },
        { effectActionDefinitionId: "ACT_LAYLA_NURSE_PS1_HEALING_LINK", targets: ["enemy:front"] },
      ],
      effectsApplied: [
        {
          unitId: "enemy:front",
          effectActionDefinitionId: "ACT_LAYLA_NURSE_PS1_ATK_DOWN",
          magnitude: -0.2,
          timeLimit: { unit: "ACTION", count: 2, owner: "EFFECT_SOURCE" },
        },
        {
          unitId: "enemy:front",
          effectActionDefinitionId: "ACT_LAYLA_NURSE_PS1_HEALING_LINK",
          magnitude: 1,
          timeLimit: { unit: "ACTION", count: 2, owner: "EFFECT_SOURCE" },
        },
      ],
      ...
      cooldowns: [
        { unitId: "ally:subject", skillDefinitionId: "SKL_LAYLA_NURSE_PS1", remaining: 2 },
      ],
    },
  },
  {
    skillDefinitionId: "SKL_LAYLA_NURSE_PS2",
    intent:
    ...
    expected: {
      actions: [
        { effectActionDefinitionId: "ACT_LAYLA_NURSE_PS2_DAMAGE", targets: ["enemy:front"] },
        { effectActionDefinitionId: "ACT_LAYLA_NURSE_PS2_EX_DOWN", targets: ["enemy:front"] },
        { effectActionDefinitionId: "ACT_LAYLA_NURSE_PS2_PP_DOWN", targets: ["enemy:front"] },
      ],
  ...
  {
    skillDefinitionId: "SKL_LAYLA_NURSE_PS3",
    intent:
    ...
    expected: {
      actions: [
        { effectActionDefinitionId: "ACT_LAYLA_NURSE_PS3_STEALTH", targets: ["ally:subject"] },
        {
          effectActionDefinitionId: "ACT_LAYLA_NURSE_PS3_THRESHOLD_DMG_DOWN",
          targets: ["ally:subject"],
        },
      ],
      effectsApplied: [
        {
          unitId: "ally:subject",
          effectActionDefinitionId: "ACT_LAYLA_NURSE_PS3_STEALTH",
          magnitude: 0,
      ...
      cooldowns: [
        { unitId: "ally:subject", skillDefinitionId: "SKL_LAYLA_NURSE_PS3", remaining: 1 },
      ],
    },
  },
];

describe("production Catalog UNIT_LAYLA_NURSE (【商売上手のエンジェルナース】レイラ・ジェンキンス)", () => {
  it.each(BEHAVIOURS)(
    "IT-UNIT-LAYLA-NURSE-001: $skillDefinitionId — $intent",
  ...
  it("IT-UNIT-LAYLA-NURSE-002: the table covers exactly the Skills the production UnitDefinition declares", () => {
  ...
  it("IT-UNIT-LAYLA-NURSE-003: every EffectAction reachable from this unit was actually executed by the table above", () => {
    resetExecutedActionIds();
