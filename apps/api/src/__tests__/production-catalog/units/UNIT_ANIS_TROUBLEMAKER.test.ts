...
describe("production Catalog UNIT_ANIS_TROUBLEMAKER (【愛を求めるトラブルメーカー】アニス・ベネット)", () => {
  it.each(MANIFESTATIONS)(
    "IT-UNIT-ANIS-TROUBLEMAKER-001: $effectActionDefinitionId ($skillDefinitionId) manifests exactly the declared effect on the $target target",
  ...
  it("IT-UNIT-ANIS-TROUBLEMAKER-002: the table's skill column covers exactly the Skills the production UnitDefinition declares", () => {
    ...
    expect(declared).toEqual([
      "SKL_ANIS_TROUBLEMAKER_AS1",
      "SKL_ANIS_TROUBLEMAKER_AS2",
      "SKL_ANIS_TROUBLEMAKER_PS1",
      "SKL_ANIS_TROUBLEMAKER_PS2",
      "SKL_ANIS_TROUBLEMAKER_EX",
    ]);
  ...
  it("IT-UNIT-ANIS-TROUBLEMAKER-100: every declared Skill activates within one completed battle, with these counts and in this order", () => {
    ...
    expect(observation.activationCounts).toEqual({
      SKL_ANIS_TROUBLEMAKER_AS1: 4,
      SKL_ANIS_TROUBLEMAKER_AS2: 4,
      SKL_ANIS_TROUBLEMAKER_PS1: 3,
      SKL_ANIS_TROUBLEMAKER_PS2: 1,
      SKL_ANIS_TROUBLEMAKER_EX: 1,
    });
    // PS1は被攻撃直前、PS2はターン開始時（1ターン目を除く）に発動する。
