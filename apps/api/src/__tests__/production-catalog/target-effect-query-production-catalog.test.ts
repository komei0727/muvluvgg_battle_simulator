...
import { createEffectInstanceId, type DomainEventId } from "../../domain/shared/event-ids.js";
...
/** PS連鎖のtrigger情報とFACTイベントフックだけを足した標準context。 */
function contextFor(
  actor: BattleUnit,
  skillId: string,
  ...
  return effectActionGroupContext({
    actor,
    skillId,
    definitions,
    recorder,
...
describe("production Catalog TARGET_HAS_EFFECT (CAP_TARGET_EFFECT_QUERY, M7-001E Issue #248)", () => {
  it("IT-CAP-TARGET-EFFECT-QUERY-PROD-001: SKL_CHIYURU_MAZE_AS1 lowers the base target's defense only while it actually holds POISON (raw「対象が毒状態だった場合一時的に防御力を20%低下させ」)", () => {
    ...
    const defDown = effectActionFrom(snapshot, "ACT_CHIYURU_MAZE_AS1_DEF_DOWN");
    expect(defDown.kind).toBe("APPLY_STAT_MOD");

    const actor = unitOf("actor", "ALLY", "UNIT_CHIYURU_MAZE", {
      column: "LEFT",
      row: "FRONT",
    });
    const run = (poisoned: boolean): readonly string[] => {
      const base = unitOf("enemy-base", "ENEMY", "UNIT_TEST_ENEMY", {
        column: "CENTER",
        row: "FRONT",
      });
      ...
      const { recorder, rootEventId } = seedRecorder("B_CAP_EFFQ");
      applyEffectActionGroups(
        plan,
        allUnits,
        contextFor(actor, skillId, definitions, recorder, rootEventId),
      );
      return completedTargetIdsOf(recorder, "ACT_CHIYURU_MAZE_AS1_DEF_DOWN");
    };
  ...
  it("IT-CAP-TARGET-EFFECT-QUERY-PROD-002: SKL_FLUTE_INFLUENCER_PS2 always heals the base amount and adds the same amount again (＋100%) plus a cleanse only for a debuffed trigger target", () => {
    ...
    const heal = effectActionFrom(snapshot, "ACT_FLUTE_INFLUENCER_PS2_HEAL");
    ...
    const actor = unitOf("actor", "ALLY", "UNIT_FLUTE_INFLUENCER", {
      column: "LEFT",
      row: "BACK",
    });
    ...
    expect(completedTargetIdsOf(withDebuff.recorder, "ACT_FLUTE_INFLUENCER_PS2_HEAL")).toEqual([
      "ally-hurt",
      "ally-hurt",
    ]);
    expect(
      completedTargetIdsOf(withDebuff.recorder, "ACT_FLUTE_INFLUENCER_PS2_REMOVE_DEBUFF"),
    ).toEqual(["ally-hurt"]);
    expect(
    ...
    expect(completedTargetIdsOf(withoutDebuff.recorder, "ACT_FLUTE_INFLUENCER_PS2_HEAL")).toEqual([
      "ally-hurt",
    ]);
    expect(
      completedTargetIdsOf(withoutDebuff.recorder, "ACT_FLUTE_INFLUENCER_PS2_REMOVE_DEBUFF"),
    ).toEqual([]);
  ...
  it("IT-CAP-TARGET-EFFECT-QUERY-PROD-003: SKL_MAIA_LAZY_AS1 shields every ally except herself only when the struck enemy holds a buff (raw「対象がバフ状態にあった場合、自身を除く味方全体にシールドを付与する」)", () => {
    ...
    const actor = unitOf("actor", "ALLY", "UNIT_MAIA_LAZY", {
      column: "LEFT",
      row: "FRONT",
    });
    const otherAlly = unitOf("ally-other", "ALLY", "UNIT_TEST_ALLY", {
      column: "CENTER",
      row: "FRONT",
    });
    const run = (buffed: boolean): readonly string[] => {
      const base = unitOf("enemy-back", "ENEMY", "UNIT_TEST_ENEMY", {
        column: "CENTER",
        row: "BACK",
      });
      const target = buffed
      ...
      return completedTargetIdsOf(recorder, "ACT_MAIA_LAZY_AS1_SHIELD");
    };
  ...
  it("IT-CAP-TARGET-EFFECT-QUERY-PROD-004: SKL_NOEL_RUMBLE_AS1 uses the +50% damage action exactly when the target is burning (raw「対象が炎上状態だった場合、この攻撃ダメージは50%増加する」)", () => {
    ...
    const plain = effectActionFrom(snapshot, "ACT_NOEL_RUMBLE_AS1_DAMAGE");
    const boosted = effectActionFrom(snapshot, "ACT_NOEL_RUMBLE_AS1_DAMAGE_VS_BURNING");
    ...
    const actor = unitOf("actor", "ALLY", "UNIT_NOEL_RUMBLE", {
      column: "LEFT",
      row: "FRONT",
    });
    const run = (burning: boolean) => {
      const base = unitOf("enemy", "ENEMY", "UNIT_TEST_ENEMY", {
        column: "CENTER",
        row: "FRONT",
      });
    ...
    expect(completedTargetIdsOf(burning, "ACT_NOEL_RUMBLE_AS1_DAMAGE_VS_BURNING")).toEqual([
      "enemy",
    ]);
    expect(completedTargetIdsOf(burning, "ACT_NOEL_RUMBLE_AS1_DAMAGE")).toEqual([]);

    // 毒（別の継続ダメージ種別）では増加しない — `continuousDamageKinds`の絞り込みが効く。
    const notBurning = run(false);
    expect(completedTargetIdsOf(notBurning, "ACT_NOEL_RUMBLE_AS1_DAMAGE")).toEqual(["enemy"]);
    expect(completedTargetIdsOf(notBurning, "ACT_NOEL_RUMBLE_AS1_DAMAGE_VS_BURNING")).toEqual([]);
  });
  ...
  it("IT-CAP-TARGET-EFFECT-QUERY-PROD-005: SKL_SHOUKA_SCHEMER_AS3's +40% applies only to an ATTACK-stat debuff, not to any other debuff (raw「対象の攻撃力がデバフがかけられていた場合、この攻撃によるダメージが40%増加する」)", () => {
    ...
    const boosted = effectActionFrom(snapshot, "ACT_SHOUKA_SCHEMER_AS3_DAMAGE_VS_ATTACK_DEBUFF");
    // 124.8 × 1.4 = 174.72。
    expect(boosted.kind === "DAMAGE" && boosted.payload.formula).toMatchObject({ power: 1.7472 });

    const actor = unitOf("actor", "ALLY", "UNIT_SHOUKA_SCHEMER", {
      column: "LEFT",
      row: "FRONT",
    });
    ...
    expect(
      completedTargetIdsOf(attackDebuffed, "ACT_SHOUKA_SCHEMER_AS3_DAMAGE_VS_ATTACK_DEBUFF"),
    ).toEqual(["enemy"]);
    expect(completedTargetIdsOf(attackDebuffed, "ACT_SHOUKA_SCHEMER_AS3_DAMAGE")).toEqual([]);

    // 防御力デバフでは増加しない（`statKinds`の絞り込みが無ければここが近似になる）。
    for (const stat of ["DEFENSE", undefined] as const) {
      const other = run(stat);
      expect(completedTargetIdsOf(other, "ACT_SHOUKA_SCHEMER_AS3_DAMAGE")).toEqual(["enemy"]);
      expect(completedTargetIdsOf(other, "ACT_SHOUKA_SCHEMER_AS3_DAMAGE_VS_ATTACK_DEBUFF")).toEqual(
        [],
      );
    }

    // 「さらに対象にかけられているバフを1つ解除する」は条件に関わらず常に適用される。
    expect(completedTargetIdsOf(attackDebuffed, "ACT_SHOUKA_SCHEMER_AS3_REMOVE_BUFF")).toEqual([
      "enemy",
    ]);
  });
  ...
  it("IT-CAP-TARGET-EFFECT-QUERY-PROD-007: SKL_CHIYURU_MAZE_EX stuns and damage-amplifies exactly the AOE targets that hold a 状態異常, poison included (raw「敵全体に威力117で攻撃する。対象が状態異常にある場合、1行動分の気絶を付与し、一度だけ対象の受ける被ダメージを100%増加させるデバフを付与する」)", () => {
    ...
    const { skill, snapshot } = loadSkill("UNIT_CHIYURU_MAZE", skillId);
    const stun = effectActionFrom(snapshot, "ACT_CHIYURU_MAZE_EX_STUN");
    const damageTakenUp = effectActionFrom(snapshot, "ACT_CHIYURU_MAZE_EX_DAMAGE_TAKEN_UP");
    ...
    const poisonedBase = unitOf("enemy-poisoned", "ENEMY", "UNIT_TEST_ENEMY", {
      column: "LEFT",
      row: "FRONT",
    });
    const stunnedBase = unitOf("enemy-stunned", "ENEMY", "UNIT_TEST_ENEMY", {
      column: "CENTER",
      row: "FRONT",
    });
    const statDebuffedBase = unitOf("enemy-statdebuff", "ENEMY", "UNIT_TEST_ENEMY", {
      column: "RIGHT",
      row: "FRONT",
    });
    const cleanEnemy = unitOf("enemy-clean", "ENEMY", "UNIT_TEST_ENEMY", {
      column: "CENTER",
      row: "BACK",
    });
    const poisoned = {
    ...
    // 攻撃自体は敵全体へ入る。
    expect([...completedTargetIdsOf(recorder, "ACT_CHIYURU_MAZE_EX_DAMAGE")].sort()).toEqual([
      "enemy-clean",
      "enemy-poisoned",
      "enemy-statdebuff",
      "enemy-stunned",
    ...
    for (const actionId of ["ACT_CHIYURU_MAZE_EX_STUN", "ACT_CHIYURU_MAZE_EX_DAMAGE_TAKEN_UP"]) {
      expect([...completedTargetIdsOf(recorder, actionId)].sort()).toEqual([
        "enemy-poisoned",
        "enemy-stunned",
      ]);
    }
    ...
    const reconstructed = reconstruct(
      initialSnapshotFor(allUnits, { include: ["effects"] }),
      recorder,
    );
    const grantedBy = (ids: readonly string[]): readonly string[] =>
      [...ids].filter((id) => id.startsWith("ACT_CHIYURU_MAZE_EX_")).sort();
    const restoredIds = (unitId: BattleUnitId): readonly string[] =>
      grantedBy(
        (reconstructed.units[unitId]?.effects ?? []).map((effect) => effect.effectDefinitionId),
      );
  ...
  it("IT-CAP-TARGET-EFFECT-QUERY-PROD-008: SKL_MERU_FLATSPIN_AS2's 状態異常 branch fires for a poisoned target too, not only for STUN/FREEZE/BLIND (raw「対象が状態異常だった場合、さらに威力62.4で追加攻撃する」)", () => {
    ...
    const actor = unitOf("actor", "ALLY", "UNIT_MERU_FLATSPIN", {
      column: "LEFT",
      row: "FRONT",
    });
    const run = (held: "poison" | "stun" | "statDebuff" | "none"): readonly string[] => {
      ...
      return completedTargetIdsOf(recorder, "ACT_MERU_FLATSPIN_AS2_DAMAGE_EXTRA");
    };
  ...
  it("IT-CAP-TARGET-EFFECT-QUERY-PROD-009: SKL_NANAE_COMMANDER_PS1 cleanses her own debuffs when she is burning, evaluated on the state left by the preceding step (raw「自身が状態異常だった場合、自身にかけられたデバフを全て解除する」)", () => {
    ...
    const run = (burning: boolean) => {
      ...
      return completedTargetIdsOf(recorder, "ACT_NANAE_COMMANDER_PS1_CLEANSE");
    };
