...
describe("production Catalog MEM_FUUKI_IINKAI (風紀委員会)", () => {
  it("IT-MEM-FUUKI-IINKAI-001: every EffectAction manifests on exactly the declared slots with the declared magnitude when the ALLY side brings the Memory", () => {
    ...
    expect(observed.actionOrder).toEqual([
      { triggeredEffectIndex: 0, actionIds: ["ACT_MEM_FUUKI_IINKAI_AFFILIATION_ATK_UP"] },
      { triggeredEffectIndex: 1, actionIds: ["ACT_MEM_FUUKI_IINKAI_ALL_SPEED_UP"] },
    ]);
  ...
  it("IT-MEM-FUUKI-IINKAI-004 [R-MEM-01] (R-MEM-01): the affiliation triggeredEffect emits no MemoryTriggered at all when no unit on the board belongs to AFF_FUUKI_IINKAI, while the party-wide one still resolves", () => {
  ...
  it("IT-MEM-FUUKI-IINKAI-005 [R-MEM-02] (R-MEM-02): keeps its API-declared slot in the resolution order when other Memories are brought alongside it, stacks onto the same slots, and its StateDeltas alone still reconstruct the started battle", () => {
