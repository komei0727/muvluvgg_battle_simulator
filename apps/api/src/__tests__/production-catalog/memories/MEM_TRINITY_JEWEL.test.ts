...
/**
 * `MEM_TRINITY_JEWEL`（Trinity Jewel）のユニット単位production結合テスト
 * （`12_テスト戦略.md`「ユニット効果軸」）。
 *
 * `AFF_TRINITY_JEWEL`に所属するキャラクターへ攻撃力+250、陣営全体へ防御力+200。
 *
...
const MEMORY_DEFINITION_ID = "MEM_TRINITY_JEWEL";

/** 所属メンバーは後衛中央の1スロットだけ。 */
const BOARD: MemoryBoardOverrides = {
  affiliationsBySlot: { BACK_CENTER: ["AFF_TRINITY_JEWEL"] },
};
...
describe("production Catalog MEM_TRINITY_JEWEL (Trinity Jewel)", () => {
  it("IT-MEM-TRINITY-JEWEL-001: every EffectAction manifests on exactly the declared slots with the declared magnitude when the ALLY side brings the Memory", () => {
    ...
    expect(observed.actionOrder).toEqual([
      { triggeredEffectIndex: 0, actionIds: ["ACT_MEM_TRINITY_JEWEL_AFFILIATION_ATK_UP"] },
      { triggeredEffectIndex: 1, actionIds: ["ACT_MEM_TRINITY_JEWEL_ALL_DEF_UP"] },
    ]);
    ...
    for (const unit of observed.started.allyUnits) {
      expect(unit.combatStats.attack).toBe(
        MEMORY_COMBAT_STATS.attack + (unit.battleUnitId === "ally:BACK_CENTER" ? 250 : 0),
      );
      expect(unit.combatStats.defense).toBe(MEMORY_COMBAT_STATS.defense + 200);
    }
  });

  it("IT-MEM-TRINITY-JEWEL-002 (R-MEM-04): the same Memory declared by the ENEMY side lands on the mirrored slots and records ENEMY as the source side, never a granter unit", () => {
  ...
  it("IT-MEM-TRINITY-JEWEL-003: every EffectAction this Memory declares was actually executed", () => {
  ...
  it("IT-MEM-TRINITY-JEWEL-004 [R-MEM-01] (R-MEM-01): the affiliation triggeredEffect emits no MemoryTriggered at all when no unit on the board belongs to AFF_TRINITY_JEWEL, while the party-wide one still resolves", () => {
    ...
    expect(observed.grants.map((grant) => grant.effectActionDefinitionId)).toEqual([
      "ACT_MEM_TRINITY_JEWEL_ALL_DEF_UP",
    ]);
    for (const unit of observed.started.allyUnits) {
