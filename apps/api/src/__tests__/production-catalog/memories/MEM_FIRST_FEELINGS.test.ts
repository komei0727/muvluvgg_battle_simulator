...
describe("production Catalog MEM_FIRST_FEELINGS (はじめてのきもち)", () => {
  ...
  it("IT-MEM-FIRST-FEELINGS-006: no memory-granted effect is removed by an unlimited BUFF/DEBUFF REMOVE_EFFECTS because every one is declared undispellable", () => {
    ...
    expect(observed.heldBefore).toEqual([
      "ACT_MEM_FIRST_FEELINGS_FRONT_CENTER_EN_DMG_UP",
      "ACT_MEM_FIRST_FEELINGS_YURIA_ATK_UP",
    ]);
