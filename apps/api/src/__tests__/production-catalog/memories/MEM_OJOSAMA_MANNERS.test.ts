...
/**
 * `MEM_OJOSAMA_MANNERS`（お嬢様直伝マナー講座）のユニット単位production結合テスト
 * （`12_テスト戦略.md`「ユニット効果軸」）。
 *
 * 陣営全体へ行動速度+80（固定値）、**敵**前衛へ被ダメージ+7.5%。Memoryが自陣営だけでなく相手陣営も対象にできる唯一の第1バッチ対象。
 *
...
const MEMORY_DEFINITION_ID = "MEM_OJOSAMA_MANNERS";
...
const EXPECTED_GRANTS: readonly MemoryGrant[] = [
  ...
  {
    effectActionDefinitionId: "ACT_MEM_OJOSAMA_MANNERS_SPEED_UP",
    unitIds: [
      "ally:FRONT_LEFT",
      "ally:FRONT_CENTER",
      "ally:FRONT_RIGHT",
...
describe("production Catalog MEM_OJOSAMA_MANNERS (お嬢様直伝マナー講座)", () => {
  ...
  it("IT-MEM-OJOSAMA-MANNERS-002 (R-MEM-04): the same Memory declared by the ENEMY side lands on the mirrored slots and records ENEMY as the source side, never a granter unit", () => {
    expect(observeMemoryGrants(MEMORY_DEFINITION_ID, "ENEMY")).toEqual(
      mirroredForEnemyDeclaration(EXPECTED_GRANTS),
    );
  });

  it("IT-MEM-OJOSAMA-MANNERS-003: every EffectAction this Memory declares was actually executed", () => {
