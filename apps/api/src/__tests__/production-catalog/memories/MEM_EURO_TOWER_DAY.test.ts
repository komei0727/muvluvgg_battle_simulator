...
describe("production Catalog MEM_EURO_TOWER_DAY (１日ユーロ・タワー体験)", () => {
  it("IT-MEM-EURO-TOWER-DAY-001 [R-MEM-01]: every EffectAction manifests on exactly the declared slots with the declared magnitude when the ALLY side brings the Memory", () => {
    ...
    expect(observed.targetSelections).toEqual([
      {
        ...
        side: "ALLY",
        count: "ALL",
        filters: [{ kind: "ROLE", role: "SUPPORT" }],
      },
      {
  ...
  it("IT-MEM-EURO-TOWER-DAY-004 [R-MEM-02] (R-MEM-02): keeps its API-declared slot in the resolution order when other Memories are brought alongside it, stacks onto the same slots, and its StateDeltas alone still reconstruct the started battle", () => {
