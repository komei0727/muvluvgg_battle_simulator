...
describe("Rule coverage ledger", () => {
  it("UT-TRACEABILITY-001: ledger contains exactly 136 rule IDs", () => {
    ...
    // TEX-009（Issue #435）が10件すべてを完了計上した（総数は変わらない）。
    ...
    // 「対象の現在HP×N%分のダメージを与える」型の攻撃が会心判定を行わない実機仕様が
    // 判明したため `R-CRT-04` を新設し140→141 — 会心不可の宣言はCatalogに無く、
    // 該当6定義はすべて`critical`を省略していた。
