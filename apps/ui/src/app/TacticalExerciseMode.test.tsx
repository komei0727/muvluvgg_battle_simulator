...
// UI-CT-053 / UI-CT-055: R-TEX-11 #2 #4 の編成プール分離とバッジ。
// UI-CT-054（プール一致の検証）はモード横断の関心事のため
// `BattleSimulatorPage.exercise.test.tsx`に残す（REF-059）。
describe("BattleSimulatorPage — unit pools (UI-CT-053/055)", () => {
...
// リロードをまたいで復元する。両モードのdraftが独立して復元されることの検証
// （「keeps the battle draft separate…」）はモード横断の関心事のため
// `BattleSimulatorPage.exercise.test.tsx`に残す（REF-059）。
...
// UI-CT-084 / UI-CT-110: Issue #539。演習の実行はログレベルではなく実行モードで
// 選ぶ。実行モード切替そのもの（UI-CT-083）とモード往復での保持（UI-CT-109）は
// モード横断の関心事のため`BattleSimulatorPage.exercise.test.tsx`に残す（REF-059）。
describe("BattleSimulatorPage — 戦術演習の実行モード (UI-CT-084/110)", () => {
...
// 結果件数を出す。他モードへ影響しないことの検証（ロックの範囲・422の枠表示）は
// モード横断の関心事のため`BattleSimulatorPage.exercise.test.tsx`に残す（REF-059）。
