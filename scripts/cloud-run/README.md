...
| `ci-deploy-candidate.sh`                                  | `deploy` job（CI）                                                | 現在productionのrevision（`status.traffic`のpercent===100、`latestReadyRevisionName`は使わない）へtraffic 100%を固定したまま、新revisionを`candidate` tag・traffic 0%で作成する。                                                  |
...
| `ci-rollback-traffic.sh`                                  | `.github/workflows/rollback-cloud-run.yml`（`workflow_dispatch`） | 指定（または、現在productionのrevisionと直近candidateのtagを除いた直近のready revisionを自動検出した）revisionへtrafficを100%戻す。                                                                                                |
