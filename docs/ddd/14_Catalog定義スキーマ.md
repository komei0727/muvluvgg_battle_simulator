...
| `CAP_STATUS_EFFECT_KIND`           | `M7-004`     | `APPLY_STATUS`の`status`がSTEALTH以外の場合の実効処理（R-STS-01〜04等）。resolverは現状`status`自体でSTEALTH以外を無条件拒否しており（ハードコード、Capability非経由）、`UNIT_LAYLA_ENTREPRENEUR`のEX/PS1（M7-001B、Issue #243でこの2行にだけ宣言）以外の既存行への遡及宣言はM7-004本体のスコープ |
...
| G-10 | 同一EffectSequence内のDAMAGE結果合算参照                              | `FormulaDefinition` の `sourceResult: SUM_DAMAGE_DEALT` / `SUM_DAMAGE_RECEIVED`                                                                                                                                                                                                                                                | `CAP_FORMULA`（既存を再利用、`IMPLEMENTED`）＋`CAP_SUM_DAMAGE_RESULT`（新規、`PLANNED`）。`SUM_*`の累計自体は`FormulaEvaluationContext`へ未配線のため、参照する定義は`CAP_SUM_DAMAGE_RESULT`の宣言を必須とし、Catalogロード時点で隔離する（RES-003A／Issue #257） |
