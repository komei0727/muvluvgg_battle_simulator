...
| G-04 | 効果解除                                 | `EffectActionDefinition.kind: REMOVE_EFFECTS`                                   | なし                                                            |
| G-06 | `DAMAGE_IMMUNITY`のダメージ量しきい値    | `APPLY_STATUS.payload.damageThreshold`（既存kindへのフィールド追加）            | なし（`APPLY_STATUS`の既存Capability方針を継承）                |
| G-08 | シールド付与                             | `EffectActionDefinition.kind: APPLY_SHIELD`                                     | なし                                                            |
