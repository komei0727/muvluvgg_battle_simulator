...
| フィールド        | 型                  | 必須 | 制約                                                                                                                                                                                                                                                                          |
| ----------------- | ------------------- | ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `stepCondition`   | ConditionDefinition | —    | 省略時 `TRUE`。`TARGET_STATE`/`TARGET_HAS_MARKER`/`TARGET_HAS_EFFECT` は許可しない（`targetCondition`専用スコープ）                                                                                                                                                           |
| `targetCondition` | ConditionDefinition | —    | 省略時 `TRUE`。`TRUE`/`AND`/`OR`/`NOT`/`TARGET_STATE`/`TARGET_HAS_MARKER`/`TARGET_HAS_EFFECT`/`EVENT_PAYLOAD` のみ許可し、含まれる`TARGET_STATE`/`TARGET_HAS_MARKER`/`TARGET_HAS_EFFECT`はすべてこの ACTION 自身の `target` と同じ `TargetReference` を参照しなければならない |
| `target`          | TargetReference     | ✓    | 対象参照                                                                                                                                                                                                                                                                      |
| `actions`         | object[]            | ✓    | 1件以上。定義順に解決                                                                                                                                                                                                                                                         |

`stepCondition` がfalseの場合、step全体をスキップする（`EffectStepSkipped`、直前結果を記録しない）。`stepCondition`がtrueの場合、`target`が解決した対象ごとに`targetCondition`を評価し、falseの対象だけを`actions`の適用から除外する（全対象falseなら対象0件のACTIONとして扱い、`SKIPPED`の直前結果を記録する、R-SKL-08）。この2フィールドはスキーマ上独立しているため、`stepCondition`の`TARGET_SET_COUNT`と`targetCondition`のTARGET_STATE/TARGET_HAS_MARKER/TARGET_HAS_EFFECTを同じACTIONで自由に併用できる（詳細はR-SKL-06、`TARGET_SET_COUNT`節）。`TARGET_HAS_EFFECT`を`targetCondition`へ置く用途は、`BRANCH`が条件に取れない複数体解決の対象（`TRIGGER_TARGET`等）に対して重複付与を対象ごとに防ぐことである（`Q-CAT-EFF-16`、production例: `SKL_JULIE_SNOW_PS2`）。
...
| `removeOnSourceDefeated` | boolean     | —    | 付与者の戦闘不能で解除（R-EFF-10）。`APPLY_MARKER`／`APPLY_SHIELD`専用。省略時は宣言なし扱い                      |
...
同じ契機は`APPLY_SHIELD`にも宣言できる（Issue #660）。付与者（`AppliedEffect.sourceUnitId`）が戦闘不能になった時点でこの`AppliedEffect`が失効し、`EffectExpired`（`reason: SOURCE_DEFEATED`）を発行する。`MarkerState`と異なりR-EFF-08の特殊失効条件と同じ`expireEffects`経路を共有する。

`APPLY_MARKER`／`APPLY_SHIELD`以外の`EffectActionDefinition`の`duration`へ宣言した場合はCatalogロード時点で拒否する（`UNSUPPORTED_SOURCE_DEFEATED_REMOVAL`）— それ以外の`AppliedEffect`には付与者の戦闘不能を判定する失効機構が無く（`ConditionDefinition`にもユニットの戦闘不能を判定するkindが存在しない）、宣言しても「付与自体は成功するのに付与者が倒れても何も起きない」silent partial implementationになるため。`expiration.conditions`（R-EFF-08）ではなく専用フィールドにしたのは、Marker側のexpiration機構自体が未実装（`UNSUPPORTED_MARKER_DURATION`）であり、Condition表現を導入すると同機構ごと実装する必要があるためである。
