...
Issue #41（代表10ユニットのv2 Catalog変換パイロット）で、当時のMapperでは表現できずfixtureから省略した10項目（G-01〜G-10、`18_ユニットv2Catalog変換検討.md`「パイロット実施結果」節）について、設計方針を確定し、実装するもの・見送るものを区分した。
...
| G-03 | 生存ユニット数を直接比較する条件         | `ConditionDefinition.kind: ALIVE_UNIT_COUNT`                                    | なし（`TARGET_STATE`と同様、Formulaを持たない純粋な条件のため）                                    |
...
いずれも `requiredCapabilities` は現時点で `PLANNED`（`capabilities.json`）のままとし、Mapper/schemaレベルでの受理と、対応するBattle Engineの実行（HP/リソース状態遷移、イベント発行）を分離している。これは既存の `CAP_HEAL` / `CAP_MARKER` などと同じ方針であり、Engine側の実装は各milestone Issue（DoT/シールドは #28 継続Damage・#33 Shield・SubUnitへのDamage適用、効果解除は #31 効果解除・無効化・CombatStat再計算 など）で追跡する。

### 見送ったもの（設計課題を明記し、実装を見送り）

| #    | 内容                                       | 見送り理由                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ---- | ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| G-05 | リソース「獲得量」自体を増減させるModifier | `MODIFY_RESOURCE` は一回限りの加減算であり、「1行動の間EXゲージ獲得量+50%」は将来発生する未知の獲得イベント（他スキルのEXゲージ加算など）に事後的に倍率を掛ける必要がある。これは新しいModifier評価モデル（獲得イベントへのフック）を要し、`APPLY_DAMAGE_MOD`/`APPLY_HEALING_MOD` のような「Duration付与時に確定した符号付き量を加算する」既存モデルでは表現できない。PP/EX消費・増加を扱う Issue #34（[M6] PSのPP消費・EX増加・Cooldown・中断を統合する）でリソース変更イベントの発行位置が具体化されてから、対応するEffectAction kind（例: `APPLY_RESOURCE_GAIN_MOD`）を設計する。                                                                                                                                                                                                        |
| G-07 | `APPLY_DAMAGE_MOD` の動的な相対比較条件    | 「対象HP割合が自身より低い敵にのみ与ダメージ+10%」は、`APPLY_DAMAGE_MOD` が付与時（skill使用時）に1回だけ評価される現行モデルに対し、以後発生する個々の`DAMAGE`解決のたびに、その時点の対象で条件を再評価する必要がある。単に `condition: ConditionDefinition` フィールドを追加するだけでは、条件内の `TargetReference` が「このDamageModifierが今まさに適用されようとしている対象」を指す手段（既存の `TargetReference` kindはBINDING/SELF/TRIGGER_SOURCE/TRIGGER_TARGET/LAST_ACTION_TARGETS/LAST_DAMAGED_TARGETSのみで、この用途を持たない）がなく、新しいTargetReference kindとDamage pipeline側の評価フックの両方の設計を要する。防御貫通と複数hit Damage pipelineを完成させる Issue #30（[M8] 防御貫通と複数hit Damage pipelineを完成する）でper-hit評価の設計が固まってから着手する。 |

`18_ユニットv2Catalog変換検討.md` のG-05（カリナPS2 包囲かんりょ～）・G-07（コトハPS2 起死回生）該当箇所は、Issue #41時点のfixtureのまま近似表現（該当効果を省略）を維持する。
