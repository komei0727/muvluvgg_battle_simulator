...
| `statusKinds`               | StatusKind[]       | `categories` が `STATUS` を含む場合だけ指定可能。省略時は `STATUS` カテゴリ全体（状態異常すべて）が対象。指定した場合はその状態異常種別（気絶のみ等）だけへ免疫を絞り込む（M7-001B、Issue #243、`EFFECT_IMMUNITY_STATUS_GRANULARITY`、R-EFF-03、`CAP_SPECIFIC_IMMUNITY`）。`categories` が `STATUS` を含まない場合に指定するとCatalogロード時点で拒否する |
