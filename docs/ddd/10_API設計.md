...
| `ruleId`       | string | 任意 | 違反規則または保留仕様ID。   |
...
| `422 Unprocessable Content`  | `UNSUPPORTED_RULE`             | 選択定義が保留仕様を必要とする。        |
...
- CORS、認証、TLS終端は配備環境に応じて別途定義する。

## API契約テスト

### 正常系

1. 最小編成同士、ターン数1で `200` と完了結果を返す。
2. 各陣営5体、メモリー6件、ターン数99を受理する。
3. 同じユニット定義を複数枠へ指定し、異なる `battleUnitId` を返す。
4. `options` 省略時にDETAILEDイベントを返す。
5. 同時全滅で `ALLY_WIN` と `SIMULTANEOUS_DEFEAT` を返す。
6. 初期状態へ全差分を適用すると最終状態に一致する。
7. イベントの `stateTransitionIndex` が対応する原因連番と状態バージョンを参照する。
8. PP消費と同量のEX増加をイベントと状態差分から確認できる。
9. 重複なし効果の次点が `isEffective: false` で保持され、繰り上げ後にtrueになる。
10. 行動・ターン期間効果が付与スコープでは減らず、次回以降に失効する。
...
6. 保留仕様を必要とする定義を `422 UNSUPPORTED_RULE` で返す。
7. 未知プロパティを拒否する。
