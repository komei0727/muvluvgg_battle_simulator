...
| `category`      | `BUFF`、`DEBUFF`、`STATUS_ABNORMALITY`のいずれか。状態異常はデバフの一種だが、解除・無効判定のため区別して返す。`R-STS-01`の本来の状態異常（気絶・凍結・暗闇＝`STATUS_AILMENT_KINDS`）だけを`STATUS_ABNORMALITY`とし、`STEALTH`/`EVASION`/`DAMAGE_IMMUNITY`等の対象に有利な`APPLY_STATUS`は`BUFF`とする（解除・免疫判定の`effect-category-classifier.ts`と同じ分類）。`statusKind`を持たない効果は効果量の符号でBUFF/DEBUFFへ分類する。 |
