...
編成画面が強化指定の効果を実行前に確認するための読み取り専用エンドポイントである。算出の正本をDomainの1か所（`FormationFactory`）に保つためにサーバー側で計算し、クライアントへ `R-ENH-02`〜`07`・`R-BON-01`〜`03`・`R-STA-01` を再実装させない。
...
| `rank`     | integer                 | 任意 | 0以上5以下の整数。省略時は5（`LR+5`）。`rankGrowth` を持たないユニットへの5以外の指定はアプリケーション検証の `422` として拒否。 |
| `gears`    | `GearRequest[]`         | 任意 | 0～9件、かつ同一の対象ステータスは3件まで（R-ENH-04 #1/#6）。省略時は0件。                                                       |
| `module`   | `ModuleOverrideRequest` | 任意 | モジュールの固定加算・割合補正をHP・攻撃力・防御力ごとに上書き（R-ENH-08）。省略時はR-ENH-03の既定値。                           |
...
R-ENH-08: モジュールの固定加算・割合補正をユニット単位で上書きする。全項目任意で、指定しないステータスはR-ENH-03の既定値のまま使う。

| プロパティ | 型                          | 必須 | 制約                           |
| ---------- | --------------------------- | ---- | ------------------------------ |
| `hp`       | `ModuleStatOverrideRequest` | 任意 | HPのモジュール補正上書き。     |
| `attack`   | `ModuleStatOverrideRequest` | 任意 | 攻撃力のモジュール補正上書き。 |
| `defense`  | `ModuleStatOverrideRequest` | 任意 | 防御力のモジュール補正上書き。 |

### ModuleStatOverrideRequest

| プロパティ | 型     | 必須 | 制約                                                                                             |
| ---------- | ------ | ---- | ------------------------------------------------------------------------------------------------ |
| `fixed`    | number | 任意 | 固定加算の上書き値。有限の実数（符号・上限下限なし）。省略時はR-ENH-03の既定値。                 |
| `ratio`    | number | 任意 | 割合補正の上書き値（内部表現の小数、R-NUM-01）。有限の実数（符号・上限下限なし）。省略時は0.09。 |
...
配置重複・値域・強化指定の検証、および定義IDの参照検証と `levelGrowth` 事前検証（`R-ENH-05` #5）・`rankGrowth` 事前検証（`R-ENH-07` #5）は戦闘シミュレーションと同じ経路を再利用し、同じ `422` の `code`・`path` を返す。
...
| `effectKindKey` | 重複判定（`R-STA-03`／`R-EFF-05`）で同種を識別する安定したキー。Catalog の `EffectActionDefinition.kindKey`（prefix `KIND_`）宣言由来であり、宣言の無い定義では `EffectActionDefinitionId` そのものが値になる（Issue #519、`Q-CAT-EFF-23`）。**効果アクション定義IDとは別物**で、複数の定義が同じ値を共有しうる — 定義を名指ししたいクライアントはこの値ではなく効果アクション定義ID（`EffectApplied`／`EffectExpired`／`EffectRemoved` が併せて運ぶ `effectActionDefinitionId`）を使う。                                                                                                                                                                                                                                                                                                                                |
| `statusKind`    | `APPLY_STATUS`由来の効果だけが持つ状態の種別（`EffectApplied.details.statusKind`と同じ列挙、M7-009/Issue #182）。有利な状態（`category: BUFF`）も含めて設定するため、`STATUS_ABNORMALITY`かどうかの判定には使わず`category`を見る。クライアントが`effectDefinitionId`の命名規則を解析せずに気絶・凍結・暗闇・隠密などを表示できるようにする。                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
