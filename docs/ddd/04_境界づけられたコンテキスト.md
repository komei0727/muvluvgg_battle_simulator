...
- PSの先制攻撃優先・速度順、1行動1回の制限、新規候補を効果処理単位の深さ優先で処理する連鎖解決（`R-ATM-01`〜`R-ATM-02`）
...
1行動の間は、戦闘ユニットとPSの組み合わせからなる発動済み集合を保持する。PSは、Battle Engineが発行するドメインイベントと各PSの発動タイミングを照合して候補にする。PSから新しい候補が誘発された場合は、誘発元の効果処理完了後に既存候補より先に処理し、新しい連鎖を解決してから元の候補処理へ戻る（効果処理中は候補の検出のみ行い、発動は保留する — `R-ATM-01`〜`R-ATM-02`）。発動済み集合にあるPSは候補から除外して循環を防ぐ。スキル解決途中で使用者が戦闘不能になった場合、そのスキルの未解決の効果を中断する。
...
└── battle/
    ...
    ├── resolution/ # 行動解決・PS/Memory発動・効果アクション適用（REF-064／#609でlifecycleから昇格）
    └── lifecycle/ # Battle集約本体・TurnState・状態Snapshot（`battle.ts`／`battle-state-snapshot.ts`／`turn-state.ts`のみ）
```
    ...
    → battle/targeting（battle/modelに依存）
      ...
      → battle/skill（battle/model・battle/targetingに依存）
        → battle/events（battle/model・battle/action・battle/outcomeに依存）
          ...
          → battle/effects（combatと相互import禁止）
          → battle/triggering（effectsと相互import禁止）
            → battle/resolution（combat・effects・triggering・events・action・skill・modelに依存する
              行動解決／効果解決の実務層。REF-064／#609でlifecycleから昇格）
              → battle/lifecycle（resolutionを含む上記すべてに依存する、Battle集約本体だけの
                最上位調整役）
  → formation（battle/modelにだけ依存する）
```

`triggering`・`effects`は`combat`と同格（`lifecycle`の下位、相互import禁止）で接続する。`resolution`は
`triggering`・`effects`のさらに上位、`lifecycle`の直下に位置する新規ノード（REF-064／#609）。
...
- `battle/lifecycle` はBattle集約本体（`battle.ts`／`battle-state-snapshot.ts`／`turn-state.ts`）だけを持つ最上位の一方向消費者であり、他のどの`battle/*`サブモジュールも`battle/lifecycle`に依存しない。
- `battle/resolution`（REF-064／#609で新設）は行動解決・PS/Memory発動・効果アクション適用の実務層で、`battle/combat`・`battle/effects`・`battle/triggering`・`battle/events`・`battle/action`・`battle/skill`・`battle/model`に依存してよいが、`battle/lifecycle`には依存しない（逆方向）。
- `battle/events` は`battle/model`・`battle/action`（`ReservedActionKind`等の型）・`battle/outcome`（`VictoryResult`等の型）を参照してよいが、`battle/combat`・`battle/resolution`・`battle/lifecycle`・`battle/triggering`・`battle/effects`には依存しない。Battle ObservationやHTTP event logのようなApplication/Presentation層にも依存しない。
...
「モジュールの依存順序」に示した一方向の連鎖（`model → outcome/targeting → action/skill → events → combat/effects/triggering → resolution → lifecycle`、および`model → formation`）が、上記規則の具体的な適用結果である。

上記の禁止依存は、既存のLayer間ESLint規則（`no-restricted-imports`）と同様の仕組みでモジュール間にも追加した（PR5で導入、[12\_テスト戦略.md](./12_テスト戦略.md) 参照）。`eslint.config.mjs`では、Flat configが同一ファイルへマッチする複数configの`rules`を（配列統合ではなく）後勝ちで置き換える性質を踏まえ、`domain/battle/*`の各サブモジュールごとに禁止import一覧を単一のconfigブロックへ完結させている。
...
| `catalog-integrity.ts`          | `catalog/integrity/`（REF-014でさらに分割） |
...
| `action-phase-resolver.ts`                     | `battle/lifecycle/`（PR3で分割）。**REF-064／#609で`battle/resolution/`へ再昇格**                                                                                                                                                                                                                                                                            |
| `cooldown-manipulation-application-service.ts` | `battle/lifecycle/`（`EventRecorder`と`skill-resolution-service`の双方に依存するため`action`ではなく`lifecycle`に置く）。**REF-064／#609で`battle/resolution/`へ再昇格**                                                                                                                                                                                     |
| `events/battle-state-snapshot.ts`              | `battle/lifecycle/`（`Battle`本体を直接読み取るスナップショット関数であり、`battle/events`の他ファイルは`Battle`に依存しないため）。**REF-064／#609で型定義（`BattleStateSnapshot`／`BattleUnitSnapshot`）だけを`battle/events/`へ切り戻し、`Battle`を読む`captureBattleState`／`captureUnitRoster`関数は`battle/lifecycle/battle-state-snapshot.ts`に残置** |
| `events/state-delta-reducer.ts`                | `battle/lifecycle/`（`battle-state-snapshot.ts`が定義する型を消費するため同じ配置とする）。**REF-064／#609で`battle/events/`へ再度移動**（消費する型自体を`battle/events/state-delta.ts`へ移したため、`Battle`型への依存が発生しない）                                                                                                                       |

`domain/shared/`・`domain/ports/` は現行のまま（`events/event-ids.ts`のみ`shared/`へ追加移動する）。`domain/index.ts` は内容のない1行ファイルであり、公開境界として機能しておらず、実コードからの参照もなかったため、PR5で削除した（各Layerは`domain/battle/model/battle-party.js`のようにサブモジュールへ直接importしており、Domain全体のbarrel exportを必要としない）。同じ理由で空だった`application/index.ts`・`infrastructure/index.ts`もPR5で削除した。`presentation/index.ts`は`buildServer`を再公開する実体のある公開境界であり削除対象ではない。
...
明細ルート（`battle-simulation-catalog-route.test.ts` に対応する実装）は、PR4で `build-server.ts` から `routes/catalog-route.ts` へ切り出した。
...
- 循環依存：初回のドラフトでは「`formation-bonus-calculator.ts`等を`formation/bonuses`へ、`battle-party.ts`を`battle/model`へ」置く案だったが、`battle-party.ts`・`battle-unit.ts`が`formation-bonus-calculator.ts`・`formation-input.ts`・`global-coordinate.ts`・`starting-combat-stats.ts`を直接importしており、一方で`formation-factory.ts`が`battle-party.ts`をimportするため、`formation`と`battle/model`が双方向依存になることが判明した。同様に、`events/battle-state-snapshot.ts`が`battle.ts`（`battle/lifecycle`）を直接importする一方、`battle.ts`は`events/event-recorder.ts`を利用するため、当初案のまま両方を`battle/events`へ置くと`battle/events`と`battle/lifecycle`が双方向依存になることも判明した。
  ...
  - 後者は、`Battle`本体を直接読み取る`battle-state-snapshot.ts`と、その型を消費する`state-delta-reducer.ts`を`battle/events`ではなく`battle/lifecycle`へ配置することで解消した（`battle/events`は`Battle`・`action-phase-resolver`等の`battle/lifecycle`に一切依存しなくなる）。**REF-064／#609でこの配置を見直した**: `state-delta-reducer.ts`が消費する型（`BattleStateSnapshot`／`BattleUnitSnapshot`）自体は`Battle`型を一切参照せず（`Battle`は`captureBattleState`のパラメータ型としてのみ使われる）、`battle/events/state-delta.ts`が既に持つ他の型（`ChargeState`／`CooldownState`等）とだけ結びついていたと判明したため、型定義だけを`battle/events/state-delta.ts`へ切り戻し、`state-delta-reducer.ts`も`battle/events/`へ再度移動した。`Battle`を直接読み取る`captureBattleState`／`captureUnitRoster`関数は引き続き`battle/lifecycle/battle-state-snapshot.ts`に残り、型を`battle/events`から再import（re-export）する。この結果`battle/lifecycle`はBattle集約本体（`battle.ts`／`turn-state.ts`／`battle-state-snapshot.ts`の3ファイル）だけに縮小し、行動解決・効果解決の実務層は新設`battle/resolution`（`battle/combat`・`battle/effects`・`battle/triggering`・`battle/events`・`battle/action`・`battle/skill`・`battle/model`に依存できるが`battle/lifecycle`には依存できない）へ切り出した。
  - 上記2件を反映した最終案では、「モジュールの依存順序」に示した単純な線形順序（`model → outcome/targeting → action/skill → events → combat → lifecycle`、`model → formation`）だけが成立し、これ以外の向きのimportは存在しない。`mise run check:circular`（`madge --circular`）は移動前の現行パスに対してすでに成功しており、PR2実施後・PR5実施後も同じ検査を新しいパスに対して実行し、いずれも循環依存なしを確認した（[12\_テスト戦略.md](./12_テスト戦略.md) 参照）。
