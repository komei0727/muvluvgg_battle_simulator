...
Catalogファイルの読み込みとsortをリクエストごとに行わない。
...
WorkerSimulationTask =
  ...
  | {
      ...
      requestId
      request: TacticalExerciseRequestDto     # turnLimitを持たない（R-TEX-01 #4）
      deadlineEpochMs
      expectedCatalogRevision
    }
```

モード判別子に既定値を持たせず、両モードとも明示的に指定する（`09_アプリケーション設計.md`「実行境界」）。タイムアウト、容量制御、Catalogリビジョン検査はモードによらず同じ機構を共有する。
...
```text
WorkerSimulationResult =
  | { ok: true, mode: BATTLE_SIMULATION, result: SimulateBattleResult }
  | { ok: true, mode: TACTICAL_EXERCISE, result: SimulateTacticalExerciseResult }
  | { ok: false, error: SerializedApplicationError }
```

成功結果はタスクと同じ判別子を持ち帰る。投入したモードと異なるモードの結果は実装不変条件違反として扱う。
...
| 失敗要因                                                  | 例外型                                 | 出力形式                                                        |
| --------------------------------------------------------- | -------------------------------------- | --------------------------------------------------------------- |
| ID一意性・参照切れ・型違い・EX消費量不一致・未知eventType | `CatalogIntegrityError`                | 違反1件ごとに `[<RULE>] <targetId>: <message>` の行             |
...
| `schemaVersion` が `3` でない                             | `UnsupportedCatalogSchemaVersionError` | 単一行メッセージ                                                |
...
- Unit・Memoryと表示に必要な全定義を起動時にだけ参照する。
- HTTPへ渡すread modelは表示用fieldだけを持つ。
...
単発シミュレーション（`POST /api/v1/battle-simulations`／`POST /api/v1/tactical-exercises`）は完全再現を要件としないため、この経路はseedを受け取らず`SystemRandomSource`を使い続ける。再現性が要る経路には次の`SeededRandomSource`を使う。

### SeededRandomSource

同一seedから決定的な乱数列を生成する`RandomSource`アダプター。mulberry32（32bit状態の非暗号用途PRNG）を依存ライブラリなしで実装する。

供給側はDomainの`SeededRandomSourceProvider`ポートで、`forRun(seed, runIndex)`が試行1回分の`RandomSourceFactory`を返す。

- 同一の`(seed, runIndex)`からは常に同一の乱数列を返し、それ以外の何にも依存しない。
- 編成候補を跨いで同じ`runIndex`が同じ乱数列を共有する（**共通乱数法**）。候補間比較の分散を下げるため、候補の識別子や順序をこの契約へ持ち込まない。
- 返すのが`RandomSource`ではなく`RandomSourceFactory`なのは、Battleごとに専用の`RandomSource`を生成するという既存契約を保ったまま、同じ乱数列を何度でも再生できるようにするためである。
- seed文字列は32bit整数へ畳み込み（FNV-1a）、試行ごとのサブシードはsplitmix32のfinalizerで導出する。
- `runIndex`は符号なし32bit整数（`0`〜`0xffffffff`）に限る。**この範囲でのみ**導出が32bitで可逆な変換だけで構成され、`runIndex`が異なればサブシードも必ず異なる。範囲外（小数・負数・`2 ** 32`以上）は32bit演算の暗黙変換で別々の入力が同じサブシードへ落ちるため、`RangeError`で弾く——黙って同じ乱数列を配ると、2つの試行が同一の結果を生み、分散を過小評価した統計が何の兆候もなく出る。

大量試行の統計・編成探索の評価経路だけがこのアダプターを使う。

### 一括評価の実行境界

編成候補の一括評価（UC-04）は`TACTICAL_EXERCISE_EVALUATION`モードのWorkerタスクとして、戦闘・演習と同じPool・同じタイムアウト・同じ容量制御・同じCatalogリビジョン検査で実行する。

**候補×試行の二重ループは1タスクの内側で回す。** 試行ごとにタスクを投入する構成にはしない——`WORKER_MAX_QUEUE`は本番で1であり、2件目以降が即座にキュー溢れ（503）になるためである。

seedの生成（リクエストで省略された場合）はWorker側のタスク振り分けで行う。ユースケースは乱数源を持たず、同じCommandからは常に同じ結果を返す契約を保つ。

`EVALUATION_MAX_TOTAL_RUNS`の既定値300は実測から決めた: production Catalogの演習敵4体に対する5対1の演習1回が50〜70ms、上限いっぱいの300試行が約15秒で完走する。基準にするのは本番Cloud Runが明示する`SIMULATION_TIMEOUT_MS=30秒`（`deploy/cloud-run/service.json`）であり、約50%に収まる——`SIMULATION_TIMEOUT_MS`のコード既定値（現在50秒、ローカル開発専用）ではない。この前提は`LOAD-EVAL-001`が固定する。

### 公開制御

`EVALUATION_ENDPOINT_ENABLED`が`false`（既定）の配備では、一括評価の実行系を配線せず`404 ENDPOINT_DISABLED`を返す。ローカルの分析ツール向けの実行系であり、公開インスタンスでは開かない。

**ルート登録自体は設定によらず常に行う。** 公開されるOpenAPI文書の形を`routes/`と`schemas/`だけで決める方針（未配線時も登録する`NO_EXERCISE`と同じ理由）を保ち、配備設定で文書が変わらないようにする。
...
設定は`SIMULATION_MAX_*`環境変数から`ApplicationConfig.executionLimits`へ入り、Worker Poolの`workerData`としてWorker Threadへ渡る。Worker側は総イベント数を`EventRecorder`へ、それ以外を`BattleDefinitions.executionLimits`へ載せる — 後者は戦闘1回分の不変オブジェクトであり、全解決スコープが`definitions`として参照するため、`PassiveActivationRuntime`の生成点すべてへ引数を通さずに同じ上限を配れる。確定値と根拠は`docs/運用手順.md`「Cloud Run配備構成」。
...
| `SIMULATION_MAX_EFFECT_RUNTIME_COUNTER_DEPTH` | 自己再誘発するRuntimeCounter更新の再帰depth上限。 |
| `EVALUATION_ENDPOINT_ENABLED`                 | 一括評価エンドポイントを提供するか。既定`false`。 |
| `EVALUATION_MAX_CANDIDATES`                   | 1リクエストの編成候補数上限。                     |
| `EVALUATION_MAX_TOTAL_RUNS`                   | 1リクエストの総試行数上限（候補数×試行数）。      |
...
ユニットIDやメモリーIDは機密情報ではないが、巨大ログとCatalog情報漏えいを避けるため通常ログへリクエスト・レスポンス全体を出さない。診断目的のAPIログ（`diagnosticId` で相関させる詳細記録）とサーバー運用ログも分離する。
...
| CPU                     | 1 vCPU                           | M4.5最小構成。M9の負荷試験でも据え置き（`REL-005`）。                                                                                           |
| Memory                  | 1 GiB → **2 GiB**（`REL-005`）   | main／WorkerのCatalog保持を含む初期値。最大応答24.1 MBを配備並行度で流したpeak RSSが840 MBで1 GiBに余裕が無く、M9で増量した。                   |
...
| Runtime service account | project IAM roleを持たない専用SA | allUsersへ公開されたcontainerがproject既定のCompute Engine SA（既定でroles/editor）を使わないようにし、侵害時のblast radiusをゼロ権限へ抑える。 |
...
### M9確定設定（REL-005の負荷試験結果）

M9の負荷・容量・soak試験（`REL-005`／Issue #198、測定の実体は
`apps/api/src/__tests__/load/`）の結果、上表のCPU・Memory・concurrency・timeout・
`WORKER_MAX_QUEUE`・`SHUTDOWN_GRACE_MS`はいずれも据え置きとした。加えて次を確定した。
確定値と実測値の対応表は`docs/運用手順.md`「Cloud Run配備構成」を正本とする。

| 設定                                          | M9確定値                                                     | 理由                                                                                                                                                                                                    |
| --------------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `WORKER_MIN_THREADS`／`WORKER_MAX_THREADS`    | 1／1                                                         | Piscinaの既定は`os.availableParallelism()`（host CPU数・affinity）由来であり、Cloud Runの1 vCPU（cgroup quota）を見ない。CPU limitへ合わせて固定する。Worker1＋待機1でconcurrency 2をちょうど受け切る。 |
| `SIMULATION_MAX_EVENTS`                       | 1000000                                                      | 実測の最悪値36,482件に対し27倍の余裕。据え置きのうえmanifestへ明示する。                                                                                                                                |
| `SIMULATION_MAX_PASSIVE_DEPTH`                | 8                                                            | 全29編成（5対5ミラー14・混成15）の実測必要深度4に対し2倍の余裕。据え置きのうえmanifestへ明示する。                                                                                                      |
| `SIMULATION_MAX_EFFECTS_PER_SCOPE`            | 100                                                          | 同29編成の実測必要量は54件で、旧値50は**これを下回っていた**（正常な混成編成1件が`EXECUTION_LIMIT_EXCEEDED`で落ちていた）。実測値の約1.9倍へ引き上げる。                                                |
| `SIMULATION_MAX_EFFECT_RUNTIME_COUNTER_DEPTH` | 10                                                           | production定義はこの経路へ到達しない。据え置きのうえmanifestへ明示する。                                                                                                                                |
| 圧縮担当と閾値                                | アプリケーション層／1 KiB／`Accept-Encoding`に応じてgzip・br | Cloud Runは応答を自動圧縮しない。最大応答23.1 MBがgzipで1.32 MB（5.7%）へ落ち、生成コストは実行期限に対して十分小さい。実装は`REL-006`／Issue #197が持つ依存導入と同じPRで行う。                        |
...
- OOMによる中断を防ぐため、負荷試験で99ターン・DETAILED（最大応答となるレベル）の上限シナリオを測定する。
...
3. RandomSourceが規定範囲外の値を返さない。
4. ID生成器が要求形式を満たす。
5. ApplicationErrorを正しいHTTPエラーへ変換する。
6. ログの秘匿フィールドが除外される。
7. 許可origin設定のwildcard、path、重複、不正URLを拒否する。
8. Catalog一覧read modelを起動時に1回だけ構築する。
