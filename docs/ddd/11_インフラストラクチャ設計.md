...
`EVALUATION_MAX_TOTAL_RUNS`の既定値300は実測から決めた: production Catalogの演習敵4体に対する5対1の演習1回が50〜70ms、上限いっぱいの300試行が約15秒（`SIMULATION_TIMEOUT_MS`=30秒の約50%）で完走する。この前提は`LOAD-EVAL-001`が固定する。
