...
`pnpm run validate-catalog <catalog-directory>`（`mise exec -- pnpm run validate-catalog <catalog-directory>`）で、起動時と同じ読み込みパイプライン（Read → Hash → Shape → Resolve → Semantic → Freeze、`src/infrastructure/catalog/catalog-file-loader.ts`）を単体で実行できる。CIやauthoring時にCatalogディレクトリの整合性を確認する用途で、Fastify・Worker Poolなしに完結する。
...
エラー契約（`src/infrastructure/catalog/catalog-cli.ts` の `formatCatalogValidationError`）:
