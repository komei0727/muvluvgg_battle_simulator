...
src/
  ...
  infrastructure/
    catalog/
      file-battle-catalog-loader.ts
      in-memory-battle-catalog.ts
      catalog-schema.ts
      catalog-integrity-validator.ts
    random/
...
maxQueue = bounded positive integer
```
...
メインスレッドは起動時にmanifestから期待する `catalogRevision` を読み、タスクへ含める。ワーカーのリビジョンが一致しない場合、そのタスクを実行せず `INVALID_DEFINITION` としてワーカーを再初期化する。

Catalogファイルを稼働中に置換するホットリロードは初期スコープに含めない。Catalog更新は新しいアプリケーション配備として全プロセスを再起動する。
...
`pnpm run validate-catalog <catalog-directory>`（`mise exec -- pnpm run validate-catalog <catalog-directory>`）で、起動時と同じ読み込みパイプライン（Read → Hash → Shape → Resolve → Semantic → Freeze、`src/infrastructure/catalog/runtime/catalog-file-loader.ts`）を単体で実行できる。CIやauthoring時にCatalogディレクトリの整合性を確認する用途で、Fastify・Worker Poolなしに完結する。
...
エラー契約（`src/infrastructure/catalog/runtime/catalog-cli.ts` の `formatCatalogValidationError`）:
...
圧縮の有無でAPI本文やETag相当の意味を変えない。本APIは `Cache-Control: no-store` であるため共有キャッシュを前提としない。
...
1. 設定を読み込み検証する。
2. Catalog manifestを読み込み期待リビジョンを得る。
3. ロガーとメトリクスを初期化する。
4. ワーカープールを生成して最小数をwarm upする。
5. 全ワーカー初期化を確認する。
6. Fastifyサーバーを構築する。
7. listenを開始する。
8. readinessを成功へ変更する。
...
- 水平スケール時はプロセス／コンテナ単位で複製する。
- Battle状態を共有しないため、sticky sessionは不要とする。
- 各レプリカへ同じCatalog成果物を配備する。
- CPU limitに合わせて `WORKER_MAX_THREADS` を設定する。
...
1. 正常リクエストを `200` へ変換する。
2. 型変換を行わず数値文字列を拒否する。
3. 未知プロパティを拒否する。
4. 本文上限を `413` で返す。
5. 全エラーが共通 `ErrorResponse` になる。
6. Request IDを応答とログへ引き継ぐ。
7. response schema外のプロパティを公開しない。
8. `Cache-Control: no-store` を返す。
