...
3. 所属条件を持つ11 Memory の production Catalog 変換（`triggeredEffects` 実装）は `M7-008`（Issue #176）が完了した。11件すべてが本書の `affiliationId` を `TargetFilterDefinition.kind: AFFILIATION` からそのまま参照しており、新たな所属の確認は不要だった（既存の `metadata.affiliations` を参照するだけで済んだ）。実データでの解決は `IT-CAP-MEMORY-DYNAMIC-PROD-001`〜`003`（`AFF_CHAOS_MAIDEN`／`AFF_COLORFUL_BOUQUET`／`AFF_PYXIS_MA_SOEUR`／`AFF_SIRIUS_SUGAR`／`AFF_TREBLE_QUINTET`／`AFF_TRINITY_JEWEL`／`AFF_FUUKI_IINKAI`）が検証する。今後 `metadata.affiliations` を増やす場合は、先に本書へ出典を追記してから更新する。
