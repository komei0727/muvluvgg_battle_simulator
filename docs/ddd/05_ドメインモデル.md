...
各配置枠（`BattlePartyMember`）は、R-STA-01を適用した開始時ステータス（`combatStats`）と、その算出元になったR-ENH-06の強化後基本ステータス（`enhancedBaseStats`）の両方を保持する。後者は編成補正・適性補正を**適用する前**のR-STA-01の基本値であり、強化指定のない陣営ではユニット定義の基本ステータスと一致する。`BattleUnit` が持つ `baseCombatStats`（戦闘中割合補正の再計算基準であり、補正を適用した**後**の値）とは別物である。
...
- 効果アクション定義ID
- `EffectKindKey`：`R-STA-03` の同種グループ鍵
...
`EffectKindKey` は付与時に確定してインスタンス自身が保持する。導出規則は Catalog 宣言由来であり（Issue #519、`Q-CAT-EFF-23`）、`EffectActionDefinition.kindKey` を宣言していればその値、宣言が無ければ `EffectActionDefinitionId` そのものである。これは `R-STA-03` の最強選択と `R-EFF-05` の `stacking.max` が共有する同種の単位であり、**定義の一意識別子ではない** — 複数の効果アクション定義が同じ鍵を共有するのがこの値の目的であり、定義を名指しする用途にはインスタンスが併せて保持する効果アクション定義IDを使う。`R-EFF-12` の再付与時 duration 差し替えはこの鍵ではなく効果アクション定義ID（状態異常は `statusKind`）で「同じ効果」を判定する。
...
6. スキル効果処理の外で新しい候補グループが生じた場合、スタック先頭へ積み、新しいグループをすべて解決した後に元のグループへ戻る。ただし完了イベント自身の候補は例外で、発行時に検出したうえで保留キューの解決後まで待つ（`R-ATM-01`の唯一の例外）。
7. PSの効果処理中に検出された候補グループは、その発動の保留キューへイベント発生順に積み、当該PSの効果処理完了後（`PassiveResolved`／`PassiveInterrupted` 発行後）に保留キュー → 完了イベント自身の候補の順でスタックへ積んで解決する（`R-ATM-01`〜`R-ATM-02` #3）。
...
- ユニット定義ID
- 属性、ユニットタイプ、ロール、適正
- 基本ステータス（レベル200・`LR+5`・無強化時点の値）
- 成長値 `levelGrowth`（任意。レベル1あたりのHP・攻撃力・防御力・行動速度の上昇量。R-ENH-05）
- ランク上昇量 `rankGrowth`（任意。ユニットランク1段あたりのHP・攻撃力・防御力・会心率の上昇量。R-ENH-07）
- EXゲージ最大値
- 定義順を保持したASとPS
- EXスキル
...
`EffectActionDefinition` は payload と `FormulaDefinition`、`DurationDefinition`、stacking を持つ。`EffectActionResolver` が対象・数値・適用順を解決し、その解決結果に基づいて `Battle` 集約上の実際の状態変更を行うのは `battle/resolution`（REF-064／#609で`battle/lifecycle`から昇格。モジュール構成は「モジュール所有と公開境界」を参照）である。
...
| `EnhancedBaseStatsCalculator` | 強化指定（学園レベル、現在レベル、ユニットランク、ギアカスタム）とユニット定義から強化後基本ステータスを純関数として算出する（R-ENH-01〜07）。強化指定がある陣営のユニットについて、開始時ステータス計算の基本値を差し替える。M11で実装する（ユニットランクの反映は後続Issue）。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `EffectActionResolver`        | EffectActionDefinitionの対象・数値・適用順を解決する。実際のHP、リソース、AppliedEffect、Markerの変更は、この解決結果を受け取った`battle/resolution`が行う（「モジュール所有と公開境界」参照）。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `BreakResolutionService`      | 戦術演習で敵ユニットのHPが0に到達したとき、戦闘不能に代えてブレイクを解決する。スキル効果処理中の到達は保留して当該効果処理の末尾で解決し、効果処理の外の到達は到達時点で解決する（R-TEX-03）。解決では撃破契機のトリガー候補を検出したうえで、効果解除（メモリー由来免除）、ブレイク強化、全回復を原子的に行う（候補の発動時機はR-ATM-01）。全戦闘不能判定箇所が経由する単一のシームとする。保留の記録（対象、HP0へ到達した原因イベントID、撃破元）が属するスコープは演習状態ではなく**1回のスキル効果処理**であり、`R-ATM-01` の保留キューと同じ寿命・同じ入れ子を持つ。ネストしたPS/Memoryの `EffectSequence` は自分の保留を持つ。保留は当該効果処理の末尾で解決した時点でクリアし、正常終了・中断のいずれでも解決せずに破棄することはない（R-TEX-06 #7・#8）。フレームを積むスタック自体は演習状態（`ExerciseRuntime`）が所有する — 演習状態が伝播した解決経路には必ず保留先も伝播していることを構造で保証するためであり（保留先を落とした経路はその経路だけ保留窓を作らずに即時解決し、実行時にも検出できない）、スナップショットにも `StateDelta` にも現れない。 |
...
6. PSの効果処理の外（前段・後段フェーズ、行動外トップレベルイベント）で新しい候補が生じた場合、新しい候補グループをスタック先頭へ積み、直ちに解決する。ただし完了イベント自身の候補は例外で、発行時に検出したうえで保留キューの解決後まで待つ（`R-ATM-01`の唯一の例外）。PSの効果処理中に生じた候補は、その発動の保留キューへイベント発生順に積み、当該PSの効果処理完了後に解決する（`07_戦闘ルール詳細.md`「スキル効果処理の割り込み制御」`R-ATM-01`〜`R-ATM-02`）。
7. 保留キュー、続いて完了イベント自身の候補を解決し終えたら、元のグループの続きへ戻る（`R-ATM-02` #3）。
8. スタックが空になるまで繰り返す。
9. 中断していた親の処理へ戻る。
...
- 演習中の敵ユニットは戦闘不能へ遷移させない。HPが0に到達したらブレイクとして解決し、復活まで完了させる。スキル効果処理中の到達は当該効果処理の末尾まで保留し、保留窓の間も戦闘不能として観測させない（R-TEX-06）。
...
| `EffectActionResolver`                                                         | `battle/skill`（EffectActionを対象・数値まで解決する。M7で追加。`battle/combat`はskillに依存する一方向のため、`combat`・`effects`への実際の呼び出しはskillではなく`battle/resolution`が行う。次段落参照）                                                                                                                                                 |
