...
| `RuntimeCounter`（`AppliedEffect`スコープ） | `battle/model`（M7で追加、EFF-005/Issue #162。`AppliedEffect`/`MarkerState`が共有する`EffectDurationState.counters`が実体を保持する） | スコープと現在値の読み取りのみを公開する。`EffectSequence`スコープは実行時識別子を持つResolverが存在しないためCatalogロード時に拒否する。 |
