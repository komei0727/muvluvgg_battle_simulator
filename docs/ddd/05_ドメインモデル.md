...
| `BreakResolutionService`      | 戦術演習で敵ユニットのHPが0に到達したとき、戦闘不能に代えてブレイクを解決する。スキル効果処理中の到達は保留して当該効果処理の末尾で解決し、効果処理の外の到達は到達時点で解決する（R-TEX-03）。解決では撃破契機のトリガー候補を検出したうえで、効果解除（メモリー由来免除）、ブレイク強化、全回復を原子的に行う（候補の発動時機はR-ATM-01）。全戦闘不能判定箇所が経由する単一のシームとする。保留の記録（HP0へ到達した原因イベントID）は、演習状態ではなく**1回のスキル効果処理の解決コンテキスト**（`R-ATM-01` の保留キューと同じ寿命・同じ入れ子）が持つ。ネストしたPS/Memoryの `EffectSequence` は自分の保留を持つ。保留は当該効果処理の末尾で解決した時点でクリアし、正常終了・中断のいずれでも解決せずに破棄することはない（R-TEX-06 #7・#8）。 |
