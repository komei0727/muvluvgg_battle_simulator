...
PR #238再々レビュー[P1]（`08_ドメインイベント.md`「イベント発行と処理」の順序契約——原因イベント自身のPS/Memory候補を直ちに解決してから子イベントを発生順に処理する、前倒し可能な明示的例外は`RuntimeCounterChanged`のみ）: 前段の修正は`SkillUseCompleted`/`PassiveResolved`をPS連鎖へ渡す（`action-skill-use-resolver.ts`の`passiveRuntime.onFactEvent`／`passive-activation-service.ts`の`yield`）より前にSKILL_USE単位減算を行っていたため、記録イベント順（完了→`EffectDurationReduced`→`EffectExpired`）と実際のPS/Memory候補解決順（減算の子イベントが先、完了イベント自身が最後）が逆転していた。完了イベント自身のPS連鎖を先に解決してから減算するよう順序を入れ替え、減算対象（`changes`）はその連鎖解決前のunitsスナップショットから決定する——連鎖解決後のunitsから決定すると、完了イベントに反応したPSが別の`skillUseId`で新たに付与したSKILL_USE効果まで「直前の使用分」として誤って巻き込む（PR #238再レビュー[P2]と同じ理由）。決定した`changes`は新設`applied-effect-duration.ts`の`applyEffectDurationChanges`で連鎖解決後のunitsへ適用する（対象の決定と適用を分離する）。さらに、その連鎖の中で子PS自身の完了イベントが同じ対象へ独立に同じSKILL_USE単位減算をかけ、スナップショット時点でまだ生きていたインスタンスを既に失効・除去している場合があるため（親PSと子PSはどちらも同じownerの独立した「1回のスキル使用完了」）、`emitEffectDurationReducedEvents`へ渡す前に連鎖解決後のunitsに実際にまだ存在する対象だけへ絞り込む。`UT-R-EFF-01-055`（AS/EX経路）・`UT-R-EFF-01-056`（PS自身の完了経路）が、記録イベント順と実際のPS発動順が一致することを検証する。
