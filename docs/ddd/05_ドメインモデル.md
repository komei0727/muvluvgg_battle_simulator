...
スコープによって実装マイルストーンが異なる。`SkillRuntime`スコープは、PS発動条件（発動回数、N回ごと、累計ダメージ閾値）に必要な最小範囲としてM6で実装する（レビュー再レビュー[P2]、Issue #143: `Battle`／`BattleUnit`スコープは対象を使うCatalog行が現れるまでフォローアップIssue（#149）へ委ね、Catalogロード時点で明示的に拒否する契約とした）。`AppliedEffect`／`EffectSequence`スコープは、効果インスタンス・Marker・期間・消費条件に所有される汎用Counter管理としてM7で拡張する。いずれのスコープも同じ `RuntimeCounter` モデルを使い、モデル自体を分割しない。
...
| `RuntimeCounter`（`SkillRuntime`スコープ）                    | `battle/model`（M6で追加。`SkillRuntime`の`cooldown`と同じ実体が保持する）                                                   | スコープと現在値の読み取りのみを公開する。`Battle`／`BattleUnit`スコープはフォローアップIssue（#149）。 |
