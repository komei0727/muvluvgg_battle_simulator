...
6. スキル効果処理の外で新しい候補グループが生じた場合、スタック先頭へ積み、新しいグループをすべて解決した後に元のグループへ戻る。
7. PSの効果処理中に検出された候補グループは、その発動の保留キューへイベント発生順に積み、当該PSの効果処理完了後（`PassiveResolved`／`PassiveInterrupted` 発行後）にスタックへ積んで解決する（`R-ATM-01`〜`R-ATM-02`）。
