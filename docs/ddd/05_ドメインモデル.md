...
`EffectActionDefinition` は payload と `FormulaDefinition`、`DurationDefinition`、stacking、requiredCapabilities を持つ。実際の状態変更は `EffectActionResolver` が `Battle` 集約上で行う。
...
`ConditionDefinition` は真偽値だけを返し、状態変更を行わない。
...
| `EffectActionResolver`      | EffectActionDefinitionを実行し、HP、リソース、AppliedEffect、Markerを変更する。 |
...
| `SkillRuntime`            | `battle/action`                 | クールタイム・チャージ状態の読み取りと、`action`モジュール内の更新操作のみを公開する。       |
...
| `RuntimeCounter`          | `battle/effects`（M7で追加）    | スコープと現在値の読み取りのみを公開する。                                                   |
...
| `FormationBonusCalculator`  | `formation/bonuses`                                                                                                                         |
| `AttributeAffinityPolicy`   | `formation/bonuses`                                                                                                                         |
| `PositionAptitudePolicy`    | `formation/positioning`                                                                                                                     |
| `CombatStatCalculator`      | `battle/combat`（`formation/bonuses`から一方向で再利用される。[04\_境界づけられたコンテキスト.md](./04_境界づけられたコンテキスト.md)参照） |
| `EffectStackingPolicy`      | `battle/effects`                                                                                                                            |
...
| `EffectActionResolver`      | `battle/skill`（EffectActionのdispatch／orchestration。M7で追加）                                                                           |
...
| `CooldownPolicy`            | `battle/action`                                                                                                                             |
| `DurationPolicy`            | `battle/effects`（M7で追加）                                                                                                                |
| `MarkerPolicy`              | `battle/effects`（M7で追加）                                                                                                                |
| `RuntimeCounterPolicy`      | `battle/effects`（M7で追加）                                                                                                                |
...
`EffectActionResolver`はEffectActionのdispatch/orchestrationを担い、`battle/effects`と`battle/combat`のどちらの処理を呼び出すかを決定する。`effects`と`combat`が互いをimportしないという [04\_境界づけられたコンテキスト.md](./04_境界づけられたコンテキスト.md) の規則は、この`EffectActionResolver`（`battle/skill`）を経由した呼び分けを前提とする。
