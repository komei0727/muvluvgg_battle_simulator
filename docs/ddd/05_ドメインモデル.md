...
Issue #167（TGT-004、R-TGT-08「ステルス」）: `order`適用後・`count`適用前の候補順（`resolveTargets`内部の`ordered`）に対し、第一優先対象（先頭）だけがStealth Markerを持つ場合に候補順の末尾へ移動する。Stealthは戦闘システム上ただ1つ存在する固定バフのため、`APPLY_MARKER`のCatalog固有`markerId`とは異なり、`domain/battle/model/marker-state.ts`が予約するCatalog横断の固定ID（`STEALTH_MARKER_ID`、値`MARKER_STEALTH`）で識別する（`APPLY_STATUS`の`status: STEALTH`は`AppliedEffect`側の未実装経路であり、この予約Markerとは独立している——Stealthを付与するproduction skillは`APPLY_MARKER`でこのIDを指定する）。「自身を対象とする自身のスキルには適用しない」（#6）と「条件によって対象が1体に限定されている場合は適用しない」（#7）は、候補順の件数が2件未満なら必ず一致する共通の判定に還元できる——`kind: SELF`は常に候補1件（`pool = [actor]`）にしかならず、`TRIGGER_TARGET`等のイベント由来の狭い候補集合も同様に1件へ収束しうるため、追加のkind分岐は不要（`target-selection-policy.ts`の`applyStealthRedirect`）。移動後に代替対象が存在しない場合（`count`が候補全件を要求する等）も、Stealthは消費したうえで元の対象へ発動する（#4、Q-TGT-05）。`resolveTargets`（既存の呼び出し元向け、消費情報を返さない）と、消費情報を返す新設`resolveTargetsWithStealthConsumption`（`resolveEffectSequence`が`targetBindings`解決時に使う）に分割し、後者だけが実際のMarker消費（`MarkerRemoved`/reason:`CONSUMPTION`）につながる——AS選択時のフィージビリティ判定（`hasResolvableTargets`）や`TargetsSelected`イベントpayload用の監査再解決（`resolveBindingSelections`）はconsumeを確定させてはならないため、引き続き`resolveTargets`を使う。`EffectSequencePlan`に`stealthConsumptions`（`targetBindings`解決中に検出した消費対象の配列）を追加し、AS/EX・チャージ発動・PS発動が共有する唯一の実行経路`resolveEffectSequencePlan`が、steps解決を開始する前に一括で`removeMarkers`（reason:`CONSUMPTION`、EFF-004で予約されていたが未使用だった値）を適用する。`MarkerRemovalSeedReason`から`CONSUMPTION`の除外を解いたのはこのIssueが最初の呼び出し元になったため。PR #234レビュー[P2]: 同じ`EffectSequence`内の複数`targetBindings`が同じStealth所持者を第一優先対象に選ぶ場合、R-TGT-10の定義順評価に従い最初のbindingでのみ移動・消費が成立するよう、`resolveEffectSequence`が検出済み`markerInstanceId`の集合を`resolveTargetsWithStealthConsumption`へ後続bindingの評価ごとに引き継ぐ（`applyStealthRedirect`はこの集合に含まれるMarkerを「既に消費済み」として無視する）。

PR #234レビュー[P1]: 唯一のproduction使用例`ACT_MAO_COMMITTEE_PS2_STEALTH`（大賀真桜PS2）は元々`APPLY_STATUS`/`status: STEALTH`のままだったため、この予約Marker規約とは無関係でR-TGT-08のリダイレクトが発動しなかった。`APPLY_MARKER`＋`STEALTH_MARKER_ID`へ変換し、`stack: {policy: REFRESH, max: 1}`・`linkedEffectGroupId: null`とした——元の`duration.timeLimit.unit: SKILL_USE`（`MarkerState`が未実装のためUNSUPPORTED_MARKER_DURATIONで拒否される）は`timeLimit`省略（消費されるかdispelされるまで持続）に、`linkedEffectGroupId`によるHeal/被ダメージ減少との連動失効（Marker⇔AppliedEffectの異種間カスケードは同じくMarkerで未実装）は独立Markerに、それぞれ近似した（`15_Unit_Memory変換台帳.md`の`MARKER_SKILL_USE_DURATION`/`MARKER_APPLIED_EFFECT_CROSS_TYPE_LINK`行、`apps/api/__tests__/marker-production-catalog.test.ts`の`IT-CAP-TARGET-STEALTH-REDIRECT-PROD-001`）。
