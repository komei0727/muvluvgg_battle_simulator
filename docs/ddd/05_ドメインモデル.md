...
スコープによって実装マイルストーンが異なる。`Battle`／`BattleUnit`／`SkillRuntime`スコープは、PS発動条件（発動回数、N回ごと、累計ダメージ閾値）に必要な最小範囲としてM6で実装する。`AppliedEffect`／`EffectSequence`スコープは、効果インスタンス・Marker・期間・消費条件に所有される汎用Counter管理としてM7で拡張する。両スコープとも同じ `RuntimeCounter` モデルを使い、モデル自体を分割しない。
...
| `PassiveTriggerMatcher`     | ドメインイベントとPSの発動タイミング・条件を照合する。`TriggerDefinition.condition`が参照する`RUNTIME_COUNTER`（`Battle`／`BattleUnit`／`SkillRuntime`スコープ）、Formation位置関係、除外timingもここで評価する（M6）。`ConditionEvaluator`が担う`EffectSequence`内部の分岐・失効条件（M7）とは評価対象が異なる。 |
...
| `RuntimeCounter`（`Battle`／`BattleUnit`／`SkillRuntime`スコープ） | `battle/model`（M6で追加。`SkillRuntime`の`cooldown`と同じ実体が保持する）                                                   | スコープと現在値の読み取りのみを公開する。                                                   |
| `RuntimeCounter`（`AppliedEffect`／`EffectSequence`スコープ）      | `battle/effects`（M7で追加。M6の最小実装を汎用Counter管理へ拡張する）                                                        | スコープと現在値の読み取りのみを公開する。                                                   |
...
| `RuntimeCounterPolicy`（`AppliedEffect`／`EffectSequence`スコープの更新） | `battle/effects`（M7で追加。M6の最小実装を拡張する）                                                                                                                                                             |
