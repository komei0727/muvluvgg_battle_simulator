...
| `RuntimeCounter`（`SkillRuntime`スコープ）                    | `battle/model`（M6で追加。`SkillRuntime`の`cooldown`と同じ実体が保持する）                                                   | スコープと現在値の読み取りのみを公開する。`Battle`／`BattleUnit`スコープはフォローアップIssue。 |
