...
├── contracts/    # ApplicationError、simulation・catalog・battle-log・error（外部JSON契約の正本、旧http-contract）
...
- `http-contract.ts` はSimulation・Catalog双方の型を1ファイルに持っていたが、PR4で `contracts/simulation.ts`・`contracts/catalog.ts`・`contracts/battle-log.ts`・`contracts/error.ts` へ分割した。
