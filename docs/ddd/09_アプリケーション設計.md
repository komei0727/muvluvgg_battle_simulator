...
```text
PreviewFormationStatsCommand {
  allyFormation: FormationInput            # SimulateBattleCommandと同じ制約（1..5体、メモリー0..6件、強化指定）
  enemyFormation: FormationInput
}
```

`turnLimit`・`logLevel` を持たない。Command検証は `SimulateBattleCommand` の編成部分と同じ規則をそのまま再利用する（人数、配置重複、値域、強化指定の`R-ENH-01` #3／`R-ENH-02` #1／`R-ENH-04` #2）。
