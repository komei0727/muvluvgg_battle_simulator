...
`turnLimit`・`logLevel` を持たない。Command検証は `SimulateBattleCommand` の編成部分と同じ規則を再利用する（配置重複、値域、強化指定の`R-ENH-01` #3／`R-ENH-02` #1／`R-ENH-04` #2）。
...
3. `SimulationPreflightValidator` の参照検証と `levelGrowth` 事前検証（`R-ENH-05` #5）を実行する。
