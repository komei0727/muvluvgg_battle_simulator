...
編成画面が実行前にステータスを確認するための読み取り専用ユースケース（`POST /api/v1/formation-stat-previews`）。戦闘を実行せず、`FormationFactory` が算出した各参加枠の開始時ステータスだけを返す。
...
5. 各参加枠の `BattlePartyMember.combatStats` をそのままResultへ投影する。

同じ編成・強化指定で `SimulateBattleUseCase` を実行したときの初期状態と必ず一致させるため、算出には戦闘と同じDomain関数（`FormationFactory`）だけを使い、Application層で再計算しない。`BattleUnit` は生成しない——`createBattleUnit` は `BattlePartyMember.combatStats` をそのまま写すだけで、開始時ステータスを変えないためである。`Battle` 集約も生成しない——スキル・EffectAction・Memoryの解決はプレビューの対象外（`R-MEM-03` の `triggeredEffects` は `BattleStarted` で解決されるため開始時ステータスに含まれない）であり、生成しても結果に影響しないため。
