...
- `enhancement.level` が1以上の整数、`enhancement.gears` が0～9件で各要素の列挙値が定義済み（M11）
- 陣営の `enhancement` 指定なしに `slots[].enhancement` が指定されていない（M11）

`levelGrowth` を持たないユニットへの `level ≠ 200` 指定（R-ENH-05）は、ユニット定義の解決を要するため参照検証と同じ段階で検出し、値域違反と同様に `INVALID_COMMAND` として返す。
