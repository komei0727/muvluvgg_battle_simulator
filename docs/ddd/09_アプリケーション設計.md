...
勝敗（`outcome`）は持たない。`breaks` はAssemblerが `UnitBroken` イベント（公開レベルによる間引き前）から投影する。集約は累計スコアとブレイク回数だけを状態として持ち、履歴を保持しない（R-TEX-10）。`unitSummaries` も同じ間引き前イベントから投影し、通常戦闘と同じ形で返す。
...
統計量は算出しない（Q-TEX-16）。4つの配列は同じ試行を同じ添字で指し、いずれも長さが `completedRuns` に一致する。
