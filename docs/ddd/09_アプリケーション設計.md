...
├── contracts/    # ApplicationError、http-contract（外部JSON契約の正本）
...
- `http-contract.ts` は現状Simulation・Catalog双方の型を1ファイルに持つ。PR4で `contracts/simulation`・`contracts/catalog`・`contracts/battle-log`・`contracts/error` へ分割する（実施内容は「PR 4」参照）。
...
`build-server.ts`（現行861行）は現在、Server構築・Route・CORS・request ID・content negotiation・ETag・OpenAPIを一括で保持する。PR4で次のように抽出し、`build-server.ts`はcompositionだけに縮小する。

1. Route登録を `routes/` へ抽出する（`GET /health/*`、`GET /api/v1/battle-simulation-catalog`、`POST /api/v1/battle-simulations`）。
2. CORS処理を `protocol/cors/` へ抽出する。
3. Request ID採番を `protocol/request-id/` へ抽出する。
4. Accept判定・Content-Type検証を `protocol/content-negotiation/` へ抽出する。
5. ETag生成・`If-None-Match`判定を `protocol/etag/` へ抽出する。
6. `schemas.ts`（現行1161行）をSimulation・Battle Log・Catalog・Healthの4分割にする。

`error-response-mapper.ts` は `ApplicationError` からHTTPエラーレスポンスへの変換を担い、`schemas/`・`protocol/`のどちらにも一意に属さない。PR4着手時に置き場所を決定し、決定した場所を本書へ反映する。
