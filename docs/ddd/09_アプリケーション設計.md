...
- `SimulateBattleUseCase` の入出力と処理順
...
| 対応可否検証     | Application          | 保留仕様を必要とする定義の検出           | `UNSUPPORTED_RULE`      |
...
次の保留仕様を必要とする定義が、選択された定義グラフに含まれる場合は戦闘を開始しない。

| 保留ID     | 検出対象                                       | エラー詳細             |
| ---------- | ---------------------------------------------- | ---------------------- |
| `Q-TGT-05` | 代替対象なしのステルス処理を必要とする対象選択 | 対象スキル・効果定義ID |
| `Q-EFF-04` | 特殊減衰を持つサブユニット追加ダメージ         | 対象効果定義ID         |
| `Q-EFF-05` | 未確定の凍結解除時増幅を使用する効果           | 対象効果定義ID         |
| `Q-EFF-06` | 炎上3回時の2倍処理を使用する効果               | 対象効果定義ID         |
...
保留仕様を使う定義は構造不正とは限らないため、Catalog全体を利用不能にはせず、選択された場合に `UNSUPPORTED_RULE` とする。
...
FormationFactoryは共通座標、配置適性、編成ボーナス、メモリー補正および開始時ステータスを計算する。同じ定義を持つ二つの参加枠も、別々の戦闘ユニットとして生成する。
...
| `UNSUPPORTED_RULE`             | 保留仕様を必要とする定義               | 定義選択の変更で可能   | しない           |
...
5. 選択された定義グラフに保留仕様があれば、Battle生成前に拒否する。
