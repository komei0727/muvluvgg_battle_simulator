...
- M7相当の不完全変換62行は17行まで減り、残る17行はすべてOPEN Taskが所有する。うち5行（`EFFECT_APPLIED_CLASSIFICATION_PAYLOAD`）と2行（`LINKED_EFFECT_GROUP_CROSS_TYPE`）、1行（`LINKED_EFFECT_GROUP_SHIELD`→`M8`）は本監査が根拠付きで再分類した。
