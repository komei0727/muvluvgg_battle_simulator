...
- Skill中断との接続
- PS発動条件用の最小`RuntimeCounter`（`SkillRuntime`スコープ。`Battle`／`BattleUnit`スコープはフォローアップIssueへ委ね、Catalogロード時点で明示的に拒否する — レビュー再レビュー[P2]）。発動回数、N回ごと（`modulo`）、累計ダメージ閾値超過回数と端数繰越を扱う（`RUNTIME_COUNTER_MODULO`、`CUMULATIVE_DAMAGE_THRESHOLD_TRIGGER`、Issue #143）
...
- `linkedEffectGroup`
- `MarkerState`
- `RuntimeCounter`の`AppliedEffect`／`EffectSequence`スコープへの拡張（M6で追加した`SkillRuntime`スコープの最小実装を、Marker・期間・消費条件に所有される汎用Counter管理へ広げる）。`Battle`／`BattleUnit`スコープは当初M6の対象としていたが、対象を使うCatalog行が現れるまでフォローアップIssueへ委ねてスコープを縮小したため（レビュー再レビュー[P2]、Issue #143）、本マイルストーンの対象にも含めない — 実装時期は対象行が確定した時点で別途計画する。
...
- `R-EFF-11`（RuntimeCounter）が`AppliedEffect`／`EffectSequence`スコープを含めて台帳上で完了する。`Battle`／`BattleUnit`スコープは対象を使うCatalog行が現れた時点のフォローアップIssueで別途完了計上する（M6でスコープを縮小したため、本マイルストーンの完了条件には含めない — レビュー再レビュー[P2]、Issue #143）。
