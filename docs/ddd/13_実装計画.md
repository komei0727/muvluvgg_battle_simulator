...
ルール数の合計は108件とする。`M0`、`M1`、`M4`、`M4.5`、`M9` はルールを支える技術・契約マイルストーンであり、独自のドメインルール数を加算しない。
...
1. production依存、`dist/`、Catalogだけを含むmulti-stage Dockerfileとcontainer ignore設定を作る。
