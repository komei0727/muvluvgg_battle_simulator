...
> **未割当ルール**: `R-MEM-01`〜`R-MEM-04`（Memory triggeredEffects候補化・解決）は下表のどのマイルストーンにも割り当てられていない。Memory発動エンジンの実装時期は未定であり、詳細は M2「スコープ外：Memory triggeredEffectsの解決」を参照。
...
Memory の本来の発動処理（`BattleStarted` イベントでの `triggeredEffects` 候補化・解決、`R-MEM-01`〜`R-MEM-04`）は Memory発動エンジンの責務だが、**現時点でこのエンジンはどのマイルストーンにも割り当てられていない**（下記「ルール割当」表に `R-MEM-*` が存在しない）。したがって、このエンジンが実装されるまでの間、戦闘開始時のMemory補正は一切適用されない状態になる。これは意図した設計後退であり、Memory発動エンジンの実装は別途マイルストーン化・Issue化が必要である。
...
- 未実装Capabilityへ到達する定義をBattle生成前に拒否する。
