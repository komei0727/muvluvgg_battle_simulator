...
- `RUNTIME_COUNTER_MODULO`（9行）、`CUMULATIVE_DAMAGE_THRESHOLD_TRIGGER`（3行）の計12行はIssue #143でproduction Catalog上も近似なしに解消済みである。`TRIGGER_POSITION_RELATION`（1行）・`TRIGGER_EXCLUSION_TIMING`（3行）はCondition APIとphase配線までIssue #144で実装済みだが、`EffectApplied`/`UnitBeingAttacked`/`HitPointReduced`の実行時発行とEffectSequenceの`TRIGGER_SOURCE`/`TRIGGER_TARGET`対象解決が必要なため、M7の`RES-005`（Issue #172）がproduction完走を担当する。`R-EFF-11`は`SkillRuntime`スコープをM6で完了し、`AppliedEffect`スコープを`EFF-005`（Issue #162）で完了した（明示的Scenarioでの検証、production定義は現状なし）。利用するproduction定義が存在しない`Battle`／`BattleUnit`スコープ、および対応するResolverが存在しない`EffectSequence`スコープはFeature Complete必須対象に含めず、Catalogロード時点で明示的に拒否する。
