...
- `RUNTIME_COUNTER_MODULO`（9行）、`CUMULATIVE_DAMAGE_THRESHOLD_TRIGGER`（3行）の計12行はIssue #143でproduction Catalog上も近似なしに解消済みである。`TRIGGER_POSITION_RELATION`（1行）・`TRIGGER_EXCLUSION_TIMING`（3行）はCondition APIとphase配線までIssue #144で実装済みだが、`EffectApplied`/`UnitBeingAttacked`/`HitPointReduced`の実行時発行とEffectSequenceの`TRIGGER_SOURCE`/`TRIGGER_TARGET`対象解決が必要なため、M7の`RES-005`（Issue #172）がproduction完走を担当する。`R-EFF-11`は`SkillRuntime`スコープをM6で完了し、`AppliedEffect`スコープを`EFF-005`（Issue #162）で完了した（明示的Scenarioでの検証、production定義は現状なし）。`EffectSequence`スコープは対応するResolverが存在しないため未実装で、`EFF-006`（Issue #212）で実装する。利用するproduction定義が存在しない`Battle`／`BattleUnit`スコープはFeature Complete必須対象に含めず、Catalogロード時点で明示的に拒否する。
...
- `linkedEffectGroup`
- `MarkerState`
- `RuntimeCounter`の`AppliedEffect`スコープへの拡張（`EFF-005`／Issue #162、完了）。利用するproduction定義が無いため明示的Scenarioを受け入れ条件として検証した（`CAP_EFFECT_RUNTIME_COUNTER`はproduction定義が現れるまで`runtimeStatus: PLANNED`のまま — `runtimeStatus: IMPLEMENTED`は`productionDefinitionIds`が非空であることを要求する`capability-definition.ts`のバリデーションと整合させるため）。`EffectSequence`スコープは対応するResolverが存在しないため未実装で、`EFF-006`（Issue #212）へ分割した。`Battle`／`BattleUnit`スコープはFeature Complete必須対象に含めず、利用定義が現れた時点で別途計画する。
...
- 最大PS深度
- 1スコープ最大効果数
- AppliedEffect RuntimeCounter自己再誘発の最大再帰depth（`PassiveChainLimits.maxEffectRuntimeCounterDepth`、R-EFF-11・PR #211再レビュー[P1]）
- コンテナCPU・メモリー
- 圧縮担当と閾値
