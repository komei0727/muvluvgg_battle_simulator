...
| `M6`           | `RuntimeCounter`（`Battle`／`BattleUnit`／`SkillRuntime`スコープの最小実装。発動回数、N回ごと`modulo`、累計ダメージ閾値をPS発動条件として評価する）、`RuntimeCounterChanged`／`RuntimeCounterReset`                                                            | `domain/battle/model`（`SkillRuntime`の`cooldown`と同じ実体に保持する状態）。`RUNTIME_COUNTER`条件の照合自体は`PassiveTriggerMatcher`（`domain/battle/triggering`）が行う。                                                                                                                                                                     |
...
- Skill中断との接続
- PS発動条件用の最小`RuntimeCounter`（`Battle`／`BattleUnit`／`SkillRuntime`スコープ）。発動回数、N回ごと（`modulo`）、累計ダメージ閾値超過回数と端数繰越を扱う（`RUNTIME_COUNTER_MODULO`、`CUMULATIVE_DAMAGE_THRESHOLD_TRIGGER`、Issue #143）
...
- `RUNTIME_COUNTER_MODULO`（9行）、`CUMULATIVE_DAMAGE_THRESHOLD_TRIGGER`（3行）、`TRIGGER_POSITION_RELATION`（1行）、`TRIGGER_EXCLUSION_TIMING`（3行）の対応テーマ16行が近似なしで解消される。`R-EFF-11`（RuntimeCounter）は本マイルストーンで`Battle`／`BattleUnit`／`SkillRuntime`スコープの受け入れ条件を満たすが、`AppliedEffect`／`EffectSequence`スコープを含む完了計上はM7で行う。
...
- `linkedEffectGroup`
- `MarkerState`
- `RuntimeCounter`の`AppliedEffect`／`EffectSequence`スコープへの拡張（M6で追加した`Battle`／`BattleUnit`／`SkillRuntime`スコープの最小実装を、Marker・期間・消費条件に所有される汎用Counter管理へ広げる）
...
- `R-EFF-11`（RuntimeCounter）が`AppliedEffect`／`EffectSequence`スコープを含めて台帳上で完了する。
