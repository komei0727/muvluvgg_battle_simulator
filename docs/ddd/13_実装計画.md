...
現在のリポジトリにはTypeScript、Vitest、ESLint、Prettierの基本設定と最小の `src/main.ts` がある。Domain、Application、HTTP、Worker、Catalogの実装はこれから開始する。
...
- 未実装または保留Capabilityを含む定義は開始前に構造化エラーで拒否する。
...
| `M3` | 最小戦闘縦切り                 | 1対1、AS、対象、ダメージ、勝敗、イベント |           10 |
| `M4` | API・Worker Walking Skeleton   | HTTP→Worker→UseCase→Battle→JSON          |            0 |
| `M5` | 行動ライフサイクル             | EX、待機、速度再計算、CT、チャージ       |            2 |
| `M6` | PSイベントエンジン             | 発動照合、先制攻撃、即時連鎖、PP・EX     |           11 |
| `M7` | 効果・状態異常・高度ターゲット | 期間、重複、解除、気絶、凍結、暗闇       |           23 |
| `M8` | 高度ダメージ                   | 防御貫通、シールド、サブ、リンク、継続   |           14 |
| `M9` | 完成・性能・リリース準備       | 全契約、実Catalog、負荷試験、運用        |            0 |

ルール数の合計は80件とする。`M0`、`M1`、`M4`、`M9` はルールを支える技術・契約マイルストーンであり、独自のドメインルール数を加算しない。
...
`M4` の配信経路と `M5` 以降のDomain拡張は、`M3` の契約が安定した後なら並行して進められる。各Domain拡張はApplicationとAPIの同じ縦切りへ結合し、長期間未結合のブランチを作らない。
...
### M7：効果・状態異常・高度ターゲット 23件

- `R-ORD-01`、`R-ORD-04`
- `R-ACT-01`～`R-ACT-02`
- `R-TGT-03`～`R-TGT-06`、`R-TGT-08`
- `R-HIT-02`～`R-HIT-03`
- `R-DMG-02`
- `R-SKL-05`
- `R-STS-01`～`R-STS-04`
...
5. Unit／Scenario／Contract／Integrationテストは当初 `src/**/*.test.ts` として対象モジュール付近へ配置し、共通支援を `src/testing/` に作る。負荷試験は別の実行設定へ分離する。
6. `SequenceRandomSource`、固定ID生成器、Manual Clockを作る。
7. 80ルールIDを抽出するトレーサビリティ台帳と検証スクリプトを作る。
8. format、typecheck、lint、test、buildをCIで実行する。
9. Fastify、Piscina、Swagger関連パッケージの互換バージョンを確認しlockfileへ追加する。
10. `src/main.ts` をComposition Rootから起動する形へ変更する準備を行う。
...
- 10ルールが台帳上で完了する。AS選択、Skill解決、リソース消費など後続依存を持つルールは部分実装として扱い、この段階では完了計上しない。
...
- 2ルールが台帳上で完了する。状態異常やPSへ依存する行動選択・速度再計算・リソース消費・チャージ阻害は後続マイルストーンで完了計上する。
...
効果インスタンス、重複、期間、解除、状態異常と残りの対象選択方式を実装する。
...
- 最も遠い、隣接、目の前、列優先
- ステルスの通常処理
- Q-TGT-05、Q-EFF-05のCapability拒否
...
- 保留仕様へ到達する定義をBattle生成前に拒否する。

## M8 高度ダメージ

### 目的

防御貫通、シールド、サブユニット、リンク、継続ダメージを既存のダメージパイプラインへ追加する。
...
- 毒の現在HP割合・付与時攻撃力上限
- 毒再付与時の期間・効果量統合
- Q-EFF-04、Q-EFF-06のCapability拒否
...
- 毒上限とシールド非適用
- サブユニット特殊減衰の開始前拒否
- 炎上3重複時2倍Capabilityの開始前拒否

### 完了条件

- 14ルールが台帳上で完了し、合計80ルールになる。
...
参考資料からUnit・Memory Markdownへの転記、意味モデリング、ID採番、レビュー、公開は [15_Catalog作成・移行ガイド.md](./15_Catalog作成・移行ガイド.md) に従う。build toolはWebページを直接取得せず、レビュー済みMarkdownだけを入力とする。
...
- 保留Capabilityを必要としない編成は実行できる。
- 必要とする編成はBattleを一度も進行させず拒否する。
- エラーに保留IDと対象Definition IDを含める。
- 保留Capabilityが実行中に初めて発覚した場合は検証漏れとして内部エラーにする。
- 保留仕様を0%、100%、対象なしなどの仮値で代替しない。
...
| 保留仕様の漏出       | 実行中に未対応分岐到達          | requiredCapabilities事前走査   | M1以降       |
...
Domain EventはBattleの最初の更新から必要であるため、実際には `IMP-009`～`IMP-011` と並行して小さく導入し、`IMP-012` で公開形式と復元検証を完成させる。

## 計画完了条件

この実装計画を開始可能と判断する条件：

- M0～M9の依存関係と完了条件が合意されている。
- 80ルールがいずれかのDomainマイルストーンへ一意に割り当てられている。
- 保留4仕様の拒否方針が維持されている。
