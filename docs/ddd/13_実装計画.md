...
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------- |
| `M6`           | `PassiveTriggerDefinition`、`PassiveTriggerMatcher`、`PassiveCandidate`、`PassiveCandidateGroup`、`PassiveResolutionStack`、発動済みPS集合、PSクールタイム                     | `domain/battle/triggering`（M6で新設）                                                        |
...
| `M7`           | `FormulaEvaluator`、`AppliedEffect`、`EffectDuration`、`EffectKindKey`、重複あり／なしの個別保持、`linkedEffectGroup`、`MarkerState`、`RuntimeCounter`、効果解除・状態異常解除 | `domain/battle/effects`                                                                       |
...
`triggering`・`effects`は本再編（`#132`）の時点では実体を伴わないため先行作成しない。`M6`着手時に`triggering`を、`M7`着手時に`effects`を新設する。`domain/battle/skill`（`EffectSequenceResolver`、`ConditionEvaluator`、`EffectActionResolver`）は`M7`で本格的に実装するが、PR3で`action-phase-resolver.ts`からSkill使用処理を切り出す際の受け皿として先に用意する。
