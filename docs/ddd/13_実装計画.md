...
この後退が `SimulationPreflightValidator`（R-FRM-06）実装後も黙って見過ごされないよう、`triggeredEffects` を持つ全Memoryは `requiredCapabilities` に `CAP_MEMORY_TRIGGERED_EFFECT`（`runtimeStatus: PLANNED`）を要求する。Memory発動エンジンが未実装の間、このMemoryを参照する編成は preflight で `UNSUPPORTED_RULE` として拒否され、「戦闘は開始できるがMemory効果だけ黙って未適用」という状態を防ぐ。Memory発動エンジン実装時に `capabilities.json` の該当エントリを `IMPLEMENTED` へ更新する。
