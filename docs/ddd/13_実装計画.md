...
ルール数の合計は108件とする。`M0`、`M1`、`M4`、`M9` はルールを支える技術・契約マイルストーンであり、独自のドメインルール数を加算しない。
...
全108ルールを、最初に全受け入れ条件を満たすDomainマイルストーンへ一意に割り当てる。
...
7. 108ルールIDを抽出するトレーサビリティ台帳と検証スクリプトを作る。
...
- 108ルールがいずれかのDomainマイルストーンへ一意に割り当てられている。
