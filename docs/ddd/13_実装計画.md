...
| --------- | ------ | ------------------------------------------------- | --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `REL-001` | [#202] | Capability状態と実行可能性のCatalog全体同期       | M7-010, DMG-011       | 完了。`IMPLEMENTED` Capabilityが production 経路のテストと`CLOSED` Taskを必ず持つことを`capability-status-audit.test.ts`（`UT-AUDIT-REL-001-001`／`002`）で機械化し、証跡が単体・Schema/Mapperテストだけだった4件へproduction Catalog統合テストを追加、`CAP_EFFECT_STEP_SET_CONDITION.implementationTaskId`を`DMG-003`へ付け替えた。Catalog一覧selectabilityとpreflightの一致は`selectability-preflight-production-catalog.test.ts`が実`catalog/`で固定する（catalogRevision `2026-08-06.1`） |
| `REL-002` | [#199] | 合成Smoke Unit削除とCatalog revision再生成        | REL-001               | 完了。実在Unit全件のselectable化（`DMG-006`／Issue #188）を前提に`UNIT_CI_SMOKE_TEST`と専用fixtureを除去し、`CAP_SKILL_RUNTIME_COUNTER`の実ライフサイクル証跡（`IT-CAP-SKILL-RUNTIME-003`）を実在Unit`SKL_HIIRO_LONEWOLF_PS1`へ移した。合成Unitの計数は`INTERNAL`タグ由来へ変え、再追加時に台帳更新を強制する（catalogRevision `2026-08-07.1`）                                                                                                                                               |
