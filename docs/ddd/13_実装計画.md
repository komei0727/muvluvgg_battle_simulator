...
- Skill中断との接続
- PS発動条件用の最小`RuntimeCounter`（`SkillRuntime`スコープ。`Battle`／`BattleUnit`スコープはフォローアップIssue（#149）へ委ね、Catalogロード時点で明示的に拒否する — レビュー再レビュー[P2]）。発動回数、N回ごと（`modulo`）、累計ダメージ閾値超過回数と端数繰越を扱う（`RUNTIME_COUNTER_MODULO`、`CUMULATIVE_DAMAGE_THRESHOLD_TRIGGER`、Issue #143）
- `TriggerDefinition`の位置関係条件（PS所有者から見た「目の前」など）と、発動を除外するBattle／Turn timing条件（`TRIGGER_POSITION_RELATION`、`TRIGGER_EXCLUSION_TIMING`、Issue #144）
...
- Formation上の位置関係（目の前）を条件にしたPS候補の生成・破棄（Issue #144）
- Battle開始・Turn開始・Turn終了での発動除外と、通常解決中の同じイベント種別での候補化（Issue #144）
...
- `RUNTIME_COUNTER_MODULO`（9行）、`CUMULATIVE_DAMAGE_THRESHOLD_TRIGGER`（3行）、`TRIGGER_POSITION_RELATION`（1行）、`TRIGGER_EXCLUSION_TIMING`（3行）の対応テーマ16行のうち、`RUNTIME_COUNTER_MODULO`・`CUMULATIVE_DAMAGE_THRESHOLD_TRIGGER`の計12行はIssue #143でproduction Catalog上も近似なしに解消される。`TRIGGER_POSITION_RELATION`・`TRIGGER_EXCLUSION_TIMING`の計4行は、`POSITION_RELATION`/`RESOLUTION_PHASE`のCondition API自体（Mapper・評価器・`BattleStarted`/`TurnCompleting`のPS phase配線）はIssue #144で実装済みだが、参照先の`EffectApplied`/`UnitBeingAttacked`/`HitPointReduced`をBattle Engineが実行時に発行しない点（Issue #23）と、EffectSequence stepの`TRIGGER_SOURCE`/`TRIGGER_TARGET`対象解決が未実装な点（Issue #151）により、対象production Catalog Unitでは近似なしに実行できず本マイルストーンの完了条件から除外する（レビュー指摘[P2]: 「Condition APIの実装」と「production Catalogでの近似なし実行」は別の完了条件として扱う。詳細は[12\_テスト戦略.md](./12_テスト戦略.md)「不完全変換解消の統合テスト」・[15_Unit_Memory変換台帳.md](./15_Unit_Memory変換台帳.md)参照）。`R-EFF-11`（RuntimeCounter）は本マイルストーンで`SkillRuntime`スコープの受け入れ条件を満たす（`resetScope: RESOLUTION_SCOPE`の解決スコープ終了時破棄・`RuntimeCounterReset`発行・候補解決の反復も、行動／ターン開始の各解決スコープ終了処理へ配線済み — レビュー再レビュー[P2]）。`Battle`／`BattleUnit`スコープは対象を使うCatalog行が現れるまでフォローアップIssue（#149）へ委ね、`AppliedEffect`／`EffectSequence`スコープを含む完了計上はM7で行う。
...
- `linkedEffectGroup`
- `MarkerState`
- `RuntimeCounter`の`AppliedEffect`／`EffectSequence`スコープへの拡張（M6で追加した`SkillRuntime`スコープの最小実装を、Marker・期間・消費条件に所有される汎用Counter管理へ広げる）。`Battle`／`BattleUnit`スコープは当初M6の対象としていたが、対象を使うCatalog行が現れるまでフォローアップIssue（#149）へ委ねてスコープを縮小したため（レビュー再レビュー[P2]、Issue #143）、本マイルストーンの対象にも含めない — 実装時期は対象行が確定した時点で別途計画する。
- リソース獲得量Modifier（`RESOURCE_GAIN_MOD`、`APPLY_RESOURCE_GAIN_MOD`、Issue #145）。`AppliedEffect`のinstance・期間・重複・失効モデルに統合し、`ResourceChanged`発行前に基礎量へ適用する
...
- EX獲得量+50%/-50%などリソース獲得量Modifierの単体・複数合成・行動単位Duration境界・capacity超過時の適用順（Issue #145）
...
- `R-EFF-11`（RuntimeCounter）が`AppliedEffect`／`EffectSequence`スコープを含めて台帳上で完了する。`Battle`／`BattleUnit`スコープは対象を使うCatalog行が現れた時点のフォローアップIssue（#149）で別途完了計上する（M6でスコープを縮小したため、本マイルストーンの完了条件には含めない — レビュー再レビュー[P2]、Issue #143）。
