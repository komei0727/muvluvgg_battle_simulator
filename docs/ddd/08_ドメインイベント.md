...
- **`unresolvedEffectCount`の精度**: 厳密値のみを公開する（推定値・上限値は公開しない）。中断が起きた時点で実際に開いていたACTION適用一覧（同じstepの残りtarget・EffectAction）のうち、未処理のまま残った件数を表す。まだ開始していないstep・branch・iterationは、その内容を一切見積もらず常に0として扱う。
