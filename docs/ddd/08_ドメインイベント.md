...
7. 候補スタック全体が空になり解決スコープが終了する場合は、そのスコープでリセットするRuntimeCounterについて`RuntimeCounterReset`を発行してからBattle状態のcounterを破棄する（[07\_戦闘ルール詳細.md](./07_戦闘ルール詳細.md) `R-EFF-11`参照）。`RuntimeCounterReset`自身の候補も4〜6の手順で抽出・解決し、新たな候補が生じなくなった時点で解決スコープを終了する。
...
`RuntimeCounterReset`はこの例外に含まれない。対象counterのスコープ（[07\_戦闘ルール詳細.md](./07_戦闘ルール詳細.md) `R-EFF-11`参照）が終了し、PS/Memory候補スタック全体が空になった後に発行する。そのスコープを開始したトップレベルイベント（`rootEventId`が指すイベント）を`parentEventId`とし、`RuntimeCounterChanged`と同様にcounterの`stateDelta`（reset前の値 → 破棄後の値）を単独で所有し、`stateVersionBefore`と`stateVersionAfter`が異なる値をとる。`RuntimeCounterReset`も有効な`TriggerDefinition.eventType`であるため、発行後に自身の候補を抽出・解決する（前節手順4〜6）。この解決によって新たな候補が生じなくなった時点で解決スコープを終了する。
...
`RuntimeCounterReset`はこの例外に含まれない。対象counterのスコープ（`R-EFF-11`）が終了しPS/Memory候補スタック全体が空になった後に発行し、そのスコープを開始したトップレベルイベント（`rootEventId`）を`parentEventId`として記録する。`RuntimeCounterChanged`と同様、reset分の`stateDelta`を単独で所有する。`RuntimeCounterReset`も有効な`TriggerDefinition.eventType`であり、発行後に自身の候補を抽出・解決し、新たな候補が生じなくなってから解決スコープを終了する。
