...
3. このイベントに起因するRuntimeCounter更新がある場合、`RuntimeCounterChanged`を連番採番してイベントバッファへ追加し、Battle状態のcounter値を確定する（「複合処理と状態差分の所有」の例外、次節参照）。`RuntimeCounterReset`はこのステップに含めない（後述）。
4. `PassiveTriggerMatcher` と Memory trigger matcher が、更新済みのcounter値を含む現在状態を使って、2で追加したイベントへ対応する候補を抽出する。
5. 候補があれば候補グループをスタックへ積み、`07_戦闘ルール詳細.md` の順序で直ちに解決する。
6. PSまたはMemoryから発生した子イベントも同じ手順で処理する。
7. 子の候補グループが空になったら親処理へ戻る。候補スタック全体が空になり解決スコープが終了する場合は、そのスコープでリセットするRuntimeCounterについて`RuntimeCounterReset`を発行してから破棄する（[07\_戦闘ルール詳細.md](./07_戦闘ルール詳細.md) `R-EFF-11`参照）。
...
`RuntimeCounterChanged`は、この順序の例外として扱う。`RUNTIME_COUNTER` Conditionが原因イベント自身の候補抽出時点で更新後の値を参照できるよう、`sequence`は原因イベントの直後・候補抽出より前で採番し、`parentEventId`に原因イベントのIDを記録する。`sequence`が親より大きいという不変条件（[12\_テスト戦略.md](./12_テスト戦略.md)「イベント不変条件」参照）は維持したまま、候補解決の相対順だけを他の子イベントより前倒しする。`ActionPointConsumed`などの内訳子イベントとは異なり、原因イベント自身の`stateDelta`にcounter変化を含めない。`RuntimeCounterChanged`がcounterの`stateDelta`（before/after）を単独で所有する主イベントであり、`stateVersionBefore`と`stateVersionAfter`は異なる値をとる。

`RuntimeCounterReset`はこの例外に含まれない。対象counterのスコープ（[07\_戦闘ルール詳細.md](./07_戦闘ルール詳細.md) `R-EFF-11`参照）が終了し、PS/Memory候補スタック全体が空になった後に発行する。そのスコープを開始したトップレベルイベント（`rootEventId`が指すイベント）を`parentEventId`とし、`RuntimeCounterChanged`と同様にcounterの`stateDelta`（reset前の値 → 破棄後の値）を単独で所有し、`stateVersionBefore`と`stateVersionAfter`が異なる値をとる。
...
`baseDelta` と `delta` が一致するのは、対象ユニットに有効なリソース獲得量Modifierが存在せず、かつcapacityによる打ち止めが発生しない場合に限る。Modifierとcapacity打ち止めはどちらも独立に`delta`を`baseDelta`から乖離させうるため、Modifier不在でも一致しない場合がある（例：EXゲージ95/100へ`baseDelta=10`を加える場合、Modifierが無くても`delta=5`になる）。`baseDelta` は `ResourceChanged` 自身のpayloadとして保持するため、`EFFECT_ACTION` 以外の `reason`（AP/PP消費に伴う `EX_GAIN` など、`EffectAction` を経由しない獲得を含む）でも基礎量と最終量を区別できる。capacityによる打ち止めが発生した場合は `ExtraGaugeOverflowDiscarded` を発行し、要求増加量（Modifier適用後・capacity適用前の量。Modifier不在なら `baseDelta` と同値）、実増加量（`delta` と同値）、破棄量（要求増加量－実増加量）から内訳を追跡する。評価順は「基礎量（`baseDelta`）確定 → Modifier適用 → capacity適用 → `ResourceChanged`／`ExtraGaugeOverflowDiscarded`発行」で固定する（[06\_戦闘状態遷移.md](./06_戦闘状態遷移.md)「RESOURCE_CONSUMING：リソース消費」参照）。
...
`RuntimeCounterChanged`は、原因となる状態変更イベントの子イベント（`parentEventId`）として発行するが、「複合処理と状態差分の所有」で定めるとおり、子イベントを主イベント候補解決後に発行する通常の順序の例外である。`sequence`は原因イベントの直後で採番したうえで、原因イベント自身の候補抽出より前に発行し、counterの`stateDelta`（before/after）を単独で所有する。`RUNTIME_COUNTER` Condition は更新後の値を参照する。

`RuntimeCounterReset`はこの例外に含まれない。対象counterのスコープ（`R-EFF-11`）が終了しPS/Memory候補スタック全体が空になった後に発行し、そのスコープを開始したトップレベルイベント（`rootEventId`）を`parentEventId`として記録する。`RuntimeCounterChanged`と同様、reset分の`stateDelta`を単独で所有する。
