...
`Battle`／`BattleUnit`／`SkillRuntime`スコープ（PS発動条件用の最小範囲）はM6で発行する。`AppliedEffect`／`EffectSequence`スコープ（Marker・期間・消費条件に所有される汎用Counter管理）はM7で拡張する。累計ダメージ閾値Counterが1回の更新で複数回分の閾値を超えた場合も`RuntimeCounterChanged`は1回発行し、`counter`のpayloadに更新後値と繰り越した端数を含める。
