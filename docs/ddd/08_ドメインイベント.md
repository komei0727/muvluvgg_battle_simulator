...
- 一方、`SkillUseInterrupted`／`PassiveInterrupted`を発行するかどうか自体（`SkillUseCompleted`／`PassiveResolved`との判定）は、この見積もり値の大小には依存しない別の判定（実際に1件以上のpending workが破棄されたと確認できた場合だけ中断とする）を使うため、`unresolvedEffectCount: 0`のまま中断イベントが発行されることはない。
