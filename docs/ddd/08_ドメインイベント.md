...
| `EffectDurationReduced`     | `FACT` | 行動・ターン単位効果の残り回数を減らすたび | 効果インスタンス、期間単位、前後値 |
...
### EffectDurationReduced payload（EFF-003、R-EFF-04/06）
...
`CooldownReduced`と同じ役割 — 減算そのものを独立Reducer復元可能にする。0になる減算も含めて発行する（0になった場合はこの直後に`EffectExpired`が続く）。
