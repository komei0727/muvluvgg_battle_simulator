...
`CooldownReduced`と同じ役割 — 減算そのものを独立Reducer復元可能にする。0になる減算も含めて発行する（0になった場合はこの直後に`EffectExpired`が続く）。`SKILL_USE`単位は、効果を保持するユニット（既定の`owner: EFFECT_TARGET`）自身が1回スキル使用を完了するたびに1減らす — `ACTION`/`TURN`と同じ「付与された当該使用では減算しない」規約を持つ。
