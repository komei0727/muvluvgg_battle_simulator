...
M7でリソース獲得量Modifier（`RESOURCE_GAIN_MOD`）が適用される場合、`delta` はModifier適用後の値とする。基礎量とModifier適用後量を区別する必要がある診断は `EffectActionCompleted` など既存のEffectAction系イベントのpayloadから追跡し、`ResourceChanged` 自体には基礎量を追加しない。capacity超過による破棄量は、Modifier適用の有無によらず引き続き `ExtraGaugeOverflowDiscarded` が保持する。評価順は「基礎量確定 → Modifier適用 → capacity適用 → `ResourceChanged`／`ExtraGaugeOverflowDiscarded`発行」で固定する（[06\_戦闘状態遷移.md](./06_戦闘状態遷移.md)「RESOURCE_CONSUMING：リソース消費」参照）。
