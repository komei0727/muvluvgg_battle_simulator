...
3. このイベントに対応するRuntimeCounter更新がある場合、`RuntimeCounterChanged`／`RuntimeCounterReset`を連番採番してイベントバッファへ追加し、Battle状態のcounter値を確定する（「複合処理と状態差分の所有」の例外、次節参照）。
...
`RuntimeCounterChanged`／`RuntimeCounterReset`は、この順序の例外として扱う。`RUNTIME_COUNTER` Conditionが原因イベント自身の候補抽出時点で更新後の値を参照できるよう、`sequence`は原因イベントの直後・候補抽出より前で採番し、`parentEventId`に原因イベントのIDを記録する。`sequence`が親より大きいという不変条件（[12\_テスト戦略.md](./12_テスト戦略.md)「イベント不変条件」参照）は維持したまま、候補解決の相対順だけを他の子イベントより前倒しする。
...
- `before`, `after`
- `delta`：実際に反映された変化量（Modifier適用後・capacity適用後。`after - before` と一致する）
- `baseDelta`：Modifier適用前の基礎量（Modifierが対象ユニットに存在しない場合は `delta` と同値）
- `reason`: `SKILL_COST` / `WAIT_COST` / `EX_GAIN` / `EFFECT_ACTION` / `TURN_RECOVERY`
- 原因イベントID

`baseDelta` は `ResourceChanged` 自身のpayloadとして保持するため、`EFFECT_ACTION` 以外の `reason`（AP/PP消費に伴う `EX_GAIN` など、`EffectAction` を経由しない獲得を含む）でも基礎量とModifier適用後量を区別できる。M7でリソース獲得量Modifier（`RESOURCE_GAIN_MOD`）が対象ユニットに有効な場合、`baseDelta` から `delta` への差がModifierの効果を表す。capacity超過によって要求量（Modifier適用後・capacity適用前の量）と `delta` が異なる場合、その差分は引き続き `ExtraGaugeOverflowDiscarded` の要求増加量・実増加量・破棄量から追跡する（`ExtraGaugeOverflowDiscarded` の要求増加量は、Modifier適用後・capacity適用前の量と一致する）。評価順は「基礎量（`baseDelta`）確定 → Modifier適用 → capacity適用 → `ResourceChanged`／`ExtraGaugeOverflowDiscarded`発行」で固定する（[06\_戦闘状態遷移.md](./06_戦闘状態遷移.md)「RESOURCE_CONSUMING：リソース消費」参照）。
...
RuntimeCounter更新イベントは、原因となる状態変更イベントの子イベントとして発行する。「複合処理と状態差分の所有」で定めるとおり、これは子イベントを主イベント候補解決後に発行する通常の順序の例外であり、`sequence`は原因イベントの直後で採番したうえで、原因イベント自身の候補抽出より前に発行する。`RUNTIME_COUNTER` Condition は更新後の値を参照する。
