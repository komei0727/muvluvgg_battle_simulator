...
### `unresolvedEffectCount`（`SkillUseInterrupted`／`PassiveInterrupted`共通）の性質

`unresolvedEffectCount`は「使用者（PSの場合は所有者）が解決中に戦闘不能になったことで、未着手のまま打ち切られた効果適用の**保守的な上限見積もり**」であり、実行時に確定した厳密な件数ではない（RES-003、PR #216レビューで判明）。

- 既に対象・ヒットが確定済みのstep（静的に事前解決されたACTION step）については、実際に打ち切られた適用・ヒット数をそのまま数えるため厳密。
- `BRANCH`／`RANDOM_BRANCH`／`REPEAT`を含む未着手subtreeについては、実際には適用せず、条件評価やbinding解決を静的に辿った見積もりを使う。`RANDOM_BRANCH`の分岐選択はRNG消費を伴うため後追いで確定できず、`WEIGHTED_ONE`は最大1分岐分、`INDEPENDENT`は未判定分も含めた全branch合算という安全側（実際より多く見積もる）の上限を使う。`weight`／`probability`が明示的に0で定義上絶対に選ばれ得ないbranchは見積もりから除外する。
- そのため`unresolvedEffectCount`は実際の値と一致しないことがある（常に安全側＝実際以上）。厳密な件数が必要なクライアントはこの値をそのまま信頼せず、目安として扱うこと。
- `SkillUseInterrupted`／`PassiveInterrupted`を発行するかどうか（`SkillUseCompleted`／`PassiveResolved`との判定）は、`unresolvedEffectCount`という数値そのものの大小比較ではなく、resolverが中断を検出した各箇所で計算する「その時点での見積もりが1件以上あるか」を判定に使う。これは`unresolvedEffectCount`と同じ見積もり計算（`countCandidateHits`系）に基づくため、見積もりの精度に依存する — 見積もりが常に安全側（実際以上）である限り、実際には何も失われていないのに`unresolvedEffectCount: 0`のまま中断イベントが誤って発行されることはないが、見積もり計算自体に漏れがあれば中断判定も影響を受け得る（実際に検出された不整合の修正例: PR #216再々々々々々々レビュー）。厳密な独立判定（pending execution stateの単一管理）への一本化は将来課題として残る。
