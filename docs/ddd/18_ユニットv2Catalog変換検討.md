...
| G-10 | `FormulaDefinition` に、同一 `EffectSequence` 内で複数回発生した `DAMAGE` 結果を合算して参照する手段がない（`DAMAGE_DEALT_RATIO.sourceResult` は `LAST_DAMAGE_DEALT` / `LAST_DAMAGE_RECEIVED` の直前1件のみ） | フルート EX ＃ぽよ・オア・トリート（列攻撃+条件付き追撃の合計与ダメージ60%を自己回復）                                                | fixtureは `LAST_DAMAGE_DEALT` を暫定採用しているが、追撃が発生した場合は列攻撃分の回復量が欠落する近似表現になっている |

G-08・G-09は `14_Catalog定義スキーマ.md`「Issue #6実装で判明した制約」で既に未実装と明記済みの項目が実際のunit変換で初めて具体的なユースケースにヒットした例。G-01〜G-07、G-10はIssue #41で新たに顕在化した不足。
...
上記 G-01〜G-10 は Issue #44「[Catalog] v2 EffectAction/Formula/Condition の未実装拡張を追加する」でトラッキングする。
