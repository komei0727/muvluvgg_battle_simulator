...
2. `dist/`、production依存、Catalog、コンパイル済みWorkerだけをruntime imageへ含める。
...
|    7 | `M45-UI-002`    | [#94]  | M4.5      | 一覧API client・load state・filter               | M45-API-002,003, M45-UI-001                |
|    8 | `M45-UI-003`    | [#95]  | M4.5      | 編成editor・選択dialog・draft validation         | M45-UI-002                                 |
|    9 | `M45-UI-004`    | [#96]  | M4.5      | 戦闘API client・execution state・error表示       | M45-API-003, M45-UI-001                    |
|   10 | `M45-UI-005`    | [#97]  | M4.5      | outcome・summary・event/transition/JSON          | M45-UI-004                                 |
|   11 | `M45-UI-006`    | [#98]  | M4.5      | responsive・accessibility・visual/E2E            | M45-UI-003～005                            |
|   12 | `M45-UI-007`    | [#99]  | M4.5      | GitHub Pages deploy・CSP・live smoke             | M45-API-002,003, M45-INFRA-002, M45-UI-006 |
|   13 | `M5-UI-001`     | [#100] | M5        | 行動Queue・AP/EX・Cooldown・Charge表示           | M4.5, M5 Domain                            |
|   14 | `M6-UI-001`     | [#101] | M6        | PS因果・Memory発動・PP/EX表示                    | M5-UI-001, M6 Domain                       |
|   15 | `M7-UI-001`     | [#102] | M7        | Heal集計・Effect/状態異常表示                    | M6-UI-001, M7 Domain                       |
|   16 | `M8-UI-001`     | [#103] | M8        | Shield・SubUnit・高度Damage内訳表示              | M7-UI-001, M8 Domain                       |
|   17 | `M9-UI-001`     | [#104] | M9        | 最大負荷・Security・運用hardening                | M8-UI-001, M9 release準備                  |
...
- M4.5の `M45-API-001`～`003`、`M45-INFRA-001`～`002`、`M45-UI-001`～`007`が完了している。
