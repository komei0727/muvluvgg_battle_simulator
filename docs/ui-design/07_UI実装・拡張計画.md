# UI実装・拡張計画

## 1. 目的

M4完了後、M5着手前にM4.5を設け、Unit・Memory一覧API、CORS、Cloud Run API配備、初期UI、GitHub Pages公開を完成させる。その後はDomainマイルストーンの進行に合わせて画面を拡張する。未実装機能を動作しているように見せず、利用可能になったデータだけを段階的に可視化する。

## 2. 依存関係

```mermaid
flowchart LR
    P0["M4.5 Catalog API + CORS"] --> CR["M4.5 Cloud Run + Delivery"]
    P0 --> P1["M4.5 UI Foundation"]
    P1 --> P2["M4.5 Formation editor"]
    P2 --> P3["M4.5 Simulation + Result"]
    CR --> P4["M4.5 GitHub Pages release"]
    P3 --> P4
    P4 --> M5["M5 UI expansion"]
    M5 --> M6["M6 UI expansion"]
    M6 --> M7["M7 UI expansion"]
    M7 --> M8["M8 UI expansion"]
    M8 --> M9["M9 production hardening"]
```

## 3. M4.5 Backend API・CORS

### 実装項目

- APIをHTTPSで到達可能な検証環境へ配備する。
- 全Unit・Memoryと表示用metadataを返すApplication queryを作る。
- `GET /api/v1/battle-simulation-catalog` のDTO、JSON Schema、OpenAPI、ETag/304を作る。
- main thread起動時に検証済みCatalogから不変の一覧read modelを構築し、戦闘実行Workerとは分離する。
- GitHub Pages originを限定許可するCORSをFastifyへ追加する。
- OPTIONS、GET、POST、`Content-Type`、`Accept`、`X-Request-Id`、`If-None-Match`を許可する。
- `X-Request-Id`、`Retry-After`、`ETag`をexposeする。
- 開発localhost originをproduction設定と分離する。
- CORS contract testを追加する。

### 完了条件

- 一覧APIがUnit・Memoryをdefinition ID昇順で返す。
- Skill/EffectAction/Formula等の完全定義をresponseへ公開しない。
- 同じCatalog revisionのETag条件付きGETが304を返す。
- `https://komei0727.github.io`相当の別originから一覧GET、preflight、最小POSTが成功する。
- 許可していないoriginにはCORS headerを返さない。
- credentialsなしで動作する。

UI実装だけではCORSを解決できない。proxy機能のないGitHub Pagesへ回避策を埋め込まない。

## 4. M4.5 Cloud Run・Delivery

### 採用構成

- APIはGoogle Cloud Run serviceへ配備する。
- regionは `asia-northeast1`、service名は `muvluvgg-battle-simulator-api` とする。
- container imageはArtifact Registryへ保存する。
- GitHub ActionsはWorkload Identity FederationでGoogle Cloudへ認証する。
- GitHub PagesはCloud Run serviceの公開HTTPS URLへ直接接続する。

### 実装項目

1. multi-stage Dockerfileとcontainer ignore設定を作る。
2. `apps/api/dist/`、production依存、Catalog、コンパイル済みWorkerだけをruntime imageへ含める。
3. non-root、Linux amd64、`0.0.0.0:$PORT`で起動する。
4. Cloud Runを1 vCPU、1 GiB、minimum 0、maximum 1、concurrency 2、timeout 40秒で構築する。
5. `WORKER_MAX_QUEUE=1`、`SHUTDOWN_GRACE_MS=8000`、production CORS originを設定する。
6. Artifact Registry pushとCloud Run deployのmain workflowを作る。
7. 新revisionでlive、ready、Catalog、simulation、CORS smoke testを実行する。
8. image digest／Git SHA／revisionを記録し、直前revisionへrollbackできるようにする。
9. Billing budget alertとログ保持期間を設定する。
10. Cloud Run service URLをGitHub EnvironmentのPages build設定へ渡す。

### 完了条件

- production containerがnon-rootで起動し、Cloud Run注入 `PORT`、Catalog、Worker、SIGTERMのtestを通る。
- Cloud Runはidle時にscale-to-zeroし、cold start後にCatalog GETと最小simulationを完了する。
- public invocationを許可する一方、CORSを認証とみなさず、bounded queue、timeout、maximum instanceで初期費用境界を持つ。
- 長期service account keyをGitHub Secretsへ保存しない。
- smoke test失敗時に新revisionへtrafficを確定せず、直前revisionへ戻せる。

## 5. M4.5 UI Foundation

### 実装項目

1. `ui/` workspaceを作成する（`#116` `M45-ARCH-001`で`apps/ui/`へ再配置済み）。
2. React、TypeScript、Vite、Vitest、Testing Library、E2E基盤を導入する。
3. GitHub Pages base pathとAPI URL設定を作る。
4. 採用モックからdesign tokens、AppShell、Panel、Button等を移植する。
5. 一覧API client、runtime validation、loading/error/manual retryを作る。
6. Catalog revisionとAPI Request IDを表示する。
7. PR CIへUI quality gateを追加する。

### 完了条件

- 空のUI shellをPages base pathでbuild・previewできる。
- 一覧APIからCatalogを取得するまで編成editorを有効にしない。
- 一覧API失敗・契約違反から手動再読込できる。
- UI bundleにCatalog/Skill/EffectAction定義を含めない。
- `battle-simulator-mock.html`をruntime dependencyにしない。

## 6. M4.5 Formation editor

### 実装項目

1. 味方・敵のFRONT 3 / REAR 3 slotを作る。
2. Unit selection dialog、検索、attribute/role filterを作る。
3. Memory 6 slotとselection dialogを作る。
4. turnLimit/logLevelを作る。
5. draft validatorとrequest mapperを作る。
6. keyboard/focus/画像fallbackを実装する。

### M4.5時点の明示挙動

- 一覧APIが返したUnit・Memoryをすべて選択可能にする。
- HEAL列はまだ結果領域で0となるが、この段階では結果領域自体を実装しない。

### 完了条件

- 1～5体を各陣営へ設定できる。
- 6体目を拒否する。
- DTO preview testで座標、IDs、turn、logLevelが契約どおりになる。
- keyboardだけで設定できる。

## 7. M4.5 Simulation + Result

### 実装項目

1. API client、Abort、35秒UI timeoutを作る。
2. success/error runtime validationを作る。
3. submit、cancel、rerun、dirty result stateを作る。
4. Outcome stripを作る。
5. Roster、DAMAGE/DEFENSE/HEAL/STATUS projectorを作る。
6. Event formatter registryとgeneric fallbackを作る。
7. State transition tableを作る。
8. Raw JSON view/copyを作る。
9. Catalog revision mismatchを表示する。
10. error code/JSON Pointer/diagnosticId表示を作る。

### 完了条件

- M4最小1対1を画面から実行できる。
- responseの実HPダメージ集計と最終statusがAPI fixtureに一致する。
- event、transition、JSONを確認できる。
- 422、503、504、network/CORS候補を区別して表示する。
- unknown event fixtureでクラッシュしない。

## 8. M4.5 GitHub Pages release

### 実装項目

1. Pages workflowを作る。
2. production API URLをGitHub environment variableで設定する。
3. CSP meta、favicon、title、descriptionを設定する。
4. visual regression、accessibility、mobile E2Eを完了する。
5. deployed Pagesから実API smoke testを行う。
6. 運用手順へUI deploy/rollbackを追加する。

### 完了条件

- mainから自動deployされる。
- Pages URLでasset 404がない。
- HTTPS、CORS、最小simulationが成功する。
- 前回成功artifactへrollbackできる手順がある。
- UI build revision、UI/API Catalog revision、Request IDを確認できる。

## 9. M5 行動ライフサイクル拡張

API v1の既存shapeへM5イベント・状態が追加されることを前提に、詳細表示を追加する。

### 追加表示

- action queueの生成・並べ替え
- AP消費、EX消費、待機理由
- cooldown開始・減算・完了
- charge開始・解放・キャンセル
- finalStateのresources、cooldowns、charge

### UI変更方針

- サマリの基本列は変更しない。
- Event formatter registryへM5 typeを追加する。
- Unit詳細drawerまたはevent展開内にresource変化を追加する。
- action queue専用tabは情報量を確認してから追加し、初期から空tabを置かない。

### 完了条件

- M5追加イベントをgeneric fallbackではなく意味のある文言で表示する。
- cooldown/charge状態をbattleUnitId単位で追跡できる。
- M4 fixtureが引き続き表示できる。

## 10. M6 PSイベントエンジン拡張

### 追加表示

- passive trigger/candidate/resolutionの因果関係
- PP/EX変化
- event parent/root sequenceのtree表示
- memory効果発動

### UI変更方針

- PSイベントは通常の時系列と因果treeを切り替えられるようにする候補とする。
- 診断イベント（`EFFECT_STEP_SKIPPED`／`EXTRA_GAUGE_OVERFLOW_DISCARDED`）は折りたたみ、既定表示を圧迫しない。Issue #463 でこの2種は `DETAILED` へ統合され、UIのログレベルは `SUMMARY`／`DETAILED` の2択になった（[03_API・データ連携設計.md](./03_API・データ連携設計.md) §3）。

### 完了条件

- Memory選択から効果発動eventまで追跡できる。
- parent/root sequenceの欠番を許容する。
- 詳細ログ量が増えてもeventを黙って削除しない。

## 11. M7 効果・状態異常・回復拡張

### 追加表示

- `HEAL`の実回復量集計
- effect/statusの付与、重複、解除、失効
- stun/freeze/blind等の状態
- advanced target selection
- finalState effects/cooldowns/charge

### UI変更方針

- HEALは要求量ではなく実HP回復量を表示する。集計そのものはIssue #464 でサーバー（`unitSummaries`）へ移した — クライアント側イベント集計は継続ダメージを取りこぼし、`SUMMARY`ではダメージ・回復イベント自体が届かないため、アダプタを足しても正しい値にならない。
- effect一覧はUnit詳細へ追加し、サマリ列を無制限に増やさない。
- effectKindKeyの未知値を汎用表示する。

### 実装結果（M7-009／Issue #182）

- summary adapterへ`HEAL_APPLIED`／`HEALING_TRANSFERRED`を追加した。このアダプタ方式自体はIssue #464 で撤去し、サーバー集計`unitSummaries`の参照へ置き換えた（[03_API・データ連携設計.md](./03_API・データ連携設計.md) §11）。
- イベントformatterへ付与（`EFFECT_APPLIED`）・拒否（`EFFECT_APPLICATION_REJECTED`）・失効（`EFFECT_EXPIRED`）・解除（`EFFECT_REMOVED`）・残り回数変化（`EFFECT_DURATION_REDUCED`／`EFFECT_CONSUMPTION_CHANGED`）・採用切替（`EFFECTIVE_EFFECT_CHANGED`）・ステータス再計算（`COMBAT_STAT_CHANGED`）・気絶／凍結／暗闇／回避（`STUN_DURATION_CHANGED`／`FREEZE_REMOVED`／`CHARGE_CANCELLED`／`CHARGE_HELD_BY_FREEZE`／`BLINDNESS_CHECK_RESOLVED`／`SKILL_MISSED`／`EVASION_ACTIVATED`）・回復（`HEAL_APPLIED`／`HEALING_TRANSFERRED`）を追加した。
- 「ユニット状態」タブが`finalState.units[].effects`を種別・category・`statusKind`・残りDuration・採用可否とともに一覧表示する。
- どの状態かは`EffectStateResponse.statusKind`（M7-009でAPIへ追加した任意プロパティ、[10_API設計.md](../ddd/10_API設計.md)「EffectStateResponse」）を正本とし、`effectKindKey`（現状は効果アクション定義ID）の命名規則を解析しない。
- `statusKind`はSTEALTH等の有利な状態にも設定されるため、状態異常かどうかはUIで判定せずAPIの`category`をそのまま表示する（`R-STS-01`の分類の正本はDomainの`effect-category-classifier.ts`）。`EffectApplied`のdetailsは`category`を持たないため、時系列イベントでは種別だけを中立に表示し、バフ／デバフ／状態異常の分類は「ユニット状態」タブで示す。
- Markerは専用表示を持たず、汎用fallback（`MARKER_APPLIED`等のgeneric row／レスポンスJSON）で確認する。M8完了後もこの扱いは変わっていない（DMG-010が追加した状態遷移タブの`delta-flattener.ts`は`markers`のEntityCollectionDeltaも`markerInstanceId`／`markerId`で1行へ展開するが、これはentity共通の汎用表示であり、Marker専用の時系列表示・「ユニット状態」タブでの`finalState.units[].markers`表示は無い）。専用表示はM9以降へ送る。`REL-008`（Issue #263）がMemory由来Markerを編成可能にし、`MarkerStateResponse`／`MARKER_APPLIED`の`details`が付与元として`sourceUnitId`か`sourceSide`のどちらか一方を返すようになったため、Marker専用表示を作る場合は付与者名の解決をこの2形へ分岐させる（`effect-event-formatters.ts`が`EffectApplied`側で既に行っている分岐と同じ）。

### 完了条件

- HEAL列がfixtureと一致する。（`UI-UT-SUM-001`、`UI-E2E-009`。Issue #464 でサーバー集計参照へ移行し、`UI-UT-SUM-011`〜`015`は退役した — [06_UIテスト戦略.md](./06_UIテスト戦略.md)「Summary projection」）
- 付与から失効までevent/transitionで辿れる。（イベントformatterが`effectInstanceId`をdetailsへ残し、`stateTransitions`の`effects` EntityCollectionDeltaと突き合わせられる）
- M4～M6 fixtureを後方互換表示できる。（`effects`を持たないfinalStateは「効果なし」と断定せず不明として表示する。`UI-UT-EFF-003`、`UI-CT-019`）

## 12. M8 高度ダメージ拡張

### 追加表示

- shield吸収、HP damage内訳
- defense ignore
- sub unit、cover、link/intercept
- continuous damage
- 複数hitの内訳

### UI変更方針

- 基本DAMAGEは引き続き最終 `hitPointDamage`を表示する。
- 内訳はevent詳細またはUnit分析drawerへ追加する。
- shield absorbedをDEFENSE列へ混ぜない。別指標を追加する場合は名称と集計規則を新規ADRで定義する。
- DoTのsource attributionはAPI契約を正本とし、付与者をUIで推測しない。

### 実装結果（DMG-010／Issue #191）

- M8ダメージのイベントformatterを`damage-event-formatters.ts`へ分け、`event-formatters.ts`のregistryへ合成した（共通の型・helperは`event-presentation.ts`が持ち、2ファイル間の循環importを避ける）。対象は`CRITICAL_CHECK_RESOLVED`／`DAMAGE_WILL_BE_APPLIED`／`DAMAGE_CALCULATED`／`DAMAGE_REDIRECTED`／`SHIELD_CONSUMED`／`SUB_UNIT_DAMAGED`／`HIT_POINT_REDUCED`／`DAMAGE_APPLIED`／`DAMAGE_CONVERTED_TO_HEAL`／`LINKED_DAMAGE_GENERATED`／`REFLECTED_DAMAGE_GENERATED`／`LETHAL_DAMAGE_SURVIVED`／`CONTINUOUS_DAMAGE_APPLIED`／`EFFECT_MERGED`。
- `DAMAGE_APPLIED`は「計算ダメージ（タイプありシールド吸収／タイプなしシールド吸収／サブユニット吸収／シールド迂回直撃／破棄）→ HPダメージ」の形で1文に並べ、[08\_ドメインイベント.md](../ddd/08_ドメインイベント.md)の不変条件#6（内訳の合計＝`calculatedDamage`）を読み手が突き合わせられるようにした。UI側で内訳を再計算・按分しない。
- 内訳フィールド（`typedShieldAbsorbed`等）はM4〜M7に録取したfixtureに存在しないため、すべて任意として扱い、値が無い項目は文からも消す（「吸収0」と断定しない）。値が0の項目も出さず、通常ヒットの1行を短く保つ。
- 複数hitは`hitIndex + 1`を「ヒットN」として出す。反射（`isReflectedDamage`）・リンク（`isLinkedDamage`）は命中判定を通らず`hitIndex`が常に0のため、ヒット番号ではなく由来ラベルを出す。
- 「ユニット状態」タブが`finalState.units[].shields`（プール合計）と`subUnits[]`（インスタンス単位の耐久）を表示する。`effects`と同じ後方互換規約で、契約より前のfixtureでは「なし」と断定せず不明として扱う。シールド（DMG-004）とサブユニット（DMG-005）は別々の契約で追加されたため片方だけを持つレスポンスが存在しうる。まとめて1つの不明メッセージにすると欠落側が黙って消えるので、それぞれ独立に「不明」／「なし」／実値を出す。
- `CONTINUOUS_DAMAGE_APPLIED`の内訳にはサブユニット吸収（`subUnitAbsorbed`）も含める。R-SUB-01第1項どおり`FIXED`継続ダメージはサブユニットへも吸収されるため、この項が無いと`calculatedDamage`と`hitPointDamage`の差を説明できない。Domain payloadが当該量を発行していなかったため、`ContinuousDamageApplied`へ追加した（[08\_ドメインイベント.md](../ddd/08_ドメインイベント.md)「ContinuousDamageApplied payload」）。公開ログのJSON schemaは`DamageApplied.subUnitAbsorbed`と同じく既に任意プロパティとして定義済みのため、wire契約の変更は無い。
- `delta-flattener.ts`が`EntityCollectionDelta`を件数だけでなく、`added`/`updated`/`removed`ごとに1行へ展開する。エンティティ名は`10_API設計.md`が定義するID項目（`effectInstanceId`／`subUnitInstanceId`／`markerInstanceId`／各`*DefinitionId`）から取り、定義IDの命名規則を解析しない。`updated`は`before`/`after`の差分leafだけを出し、差分が公開projectionに現れない場合は「表示可能な変更項目なし」と明示する。
- サマリ表の`DAMAGE`／`DEFENSE`列は変更していない（UI変更方針「基本DAMAGEは引き続き最終`hitPointDamage`」「shield absorbedをDEFENSE列へ混ぜない」）。`CONTINUOUS_DAMAGE_APPLIED`もsummary adapterへは追加していない — 列の追加・集計規則の変更は同方針が新規ADRを要求するため。
- 大量hit/eventについては`EventTimeline`の段階描画（初期50件＋「さらに表示」）をそのまま使う。

### 完了条件

- calculated、shield absorbed、HP damageを混同せず表示する。（`UI-UT-DMG-001`〜`021`、`UI-E2E-010`）
- subUnit/effect collection deltaを汎用JSONだけでなく意味のある表示へ変換する。（`UI-UT-DLT-001`〜`005`、`UI-UT-DMG-022`〜`025`、`UI-CT-022`〜`024`）
- 大量hit/eventで操作不能にならない。（`EventTimeline`の段階描画。M4.5から変更なし）

## 13. M9 Production hardening

### 実装項目

- production Catalog/API/UI revisionの整合監視
- response size/99 turn performance検証
- rate limit/capacity UX調整
- browser compatibility最終確認
- CSP、dependency、supply-chain確認
- 運用・rollback・障害切り分け手順
- 一覧APIと戦闘APIのrevision整合・cache運用の最終確認

### 完了条件

- 最大編成・turn 99・DETAILEDの実データを操作可能時間内に表示する。
- UIがresponseを切り捨てず、段階描画する。
- API障害とPages障害の切り分け手順がある。
- 全UI acceptance、accessibility、live E2Eが成功する。

## 13.5 M10 戦術演習UI

`TEX-001`（戦術演習シミュレータ実装）のUIフェーズ。正本は[../ddd/13\_実装計画.md](../ddd/13_実装計画.md)「M10 戦術演習」とする。

### 実装項目

- モードタブ（通常戦闘／戦術演習）の導入とモード別state分離（`UI-AC-018`、`UI-CMP-010`／`013`）
- 演習用編成UI：敵は6枠の盤面へ常に1体・敵メモリーなし・5ターン固定表示（`UI-AC-019`／`020`、`UI-CMP-011`）
- `POST /api/v1/tactical-exercises` のclient・契約ミラー型・実行時shape検証（`UI-API-014`／`015`）
- スコア・ブレイク履歴表示（`ScoreSummaryHeader`、`BreakTimeline`。`UI-AC-021`、`UI-CMP-012`）
- 演習イベントのタイムライン表示とフォールバック（`UI-AC-022`、`UI-API-016`）
- Component test `UI-CT-027`～`032` とE2Eスモーク

### 完了条件

- `UI-AC-018`～`022`と`UI-CT-027`～`032`を満たす。
- 通常戦闘の既存受け入れ条件（`UI-AC-001`～`017`）が退行しない。
- deployed APIに対する演習実行のlive E2Eが成功する。

## 13.6 M11 基本ステータス強化UI

`ENH-001`（基本ステータス強化、Issue #409）のUIフェーズ。正本は[../ddd/13\_実装計画.md](../ddd/13_実装計画.md)「M11 基本ステータス強化」とする。

### 実装項目

- 陣営強化パネル（強化トグル・学園レベル入力）の追加（`UI-AC-023`／`024`、`UI-CMP-014`）
- ユニット強化ダイアログ（レベル・ギア9枠）の追加（`UI-AC-025`／`026`、`UI-CMP-015`）
- `enhancement`のdraft拡張・リクエスト変換・クライアント検証（`UI-API-017`／`018`、`UI-CMP-016`）
- `enhancement`配下の422 violationのfield対応づけ（ギア送信配列から元枠への逆引きを含む。`UI-API-019`）
- Component test `UI-CT-033`～`037`

### 完了条件

- `UI-AC-023`～`026`と`UI-CT-033`～`037`を満たす。
- 強化トグルOFFの送信ペイロードが既存契約と同一であり、既存受け入れ条件が退行しない。
- 成長値（levelGrowth）を持たないユニットへのレベル指定422をサーバー違反として該当入力へ表示できる。

## 13.7 入力の永続化（Issue #425）

編成のたびに学園レベル・ユニットレベル・ギアを入力し直す負担をなくすフェーズ。正本は[01_UI要求・画面設計.md](./01_UI要求・画面設計.md) §5.9とする。API・デプロイ構成は変更しない（`UI-ADR-012`）。

### 実装項目

- storage wrapper（`lib/storage.ts`）と保存スキーマ・写像の純関数（`features/formation/persistence.ts`）
- `mlgg:last-draft` の復元と、Catalog到着後の孤児IDクリア（`UI-AC-029`）
- 味方限定のプリフィルと書き戻し（`mlgg:player-data`。`UI-AC-030`）
- リセット2操作（`FormationResetControls`、`UI-AC-031`、`UI-CMP-020`）
- Pure unit test `UI-UT-PST-001`〜`009`、component test `UI-CT-044`〜`049`

### 完了条件

- `UI-AC-029`〜`031`と`UI-CMP-019`／`020`を満たす。
- localStorageが使えない環境でも既存の受け入れ条件（`UI-AC-001`〜`028`）が退行しない。
- 保存データの破損・旧`schemaVersion`・Catalog差分で起動しても、影響範囲だけが初期化される。

## 13.8 編成枠のユニット移動（Issue #437）

配置済みユニットの位置変更に「外して選び直す」手戻りをなくすフェーズ。正本は[01_UI要求・画面設計.md](./01_UI要求・画面設計.md) §5.1とする。API・デプロイ構成は変更しない。

### 実装項目

- `unitMoved` action（同一陣営内の移動・入れ替え。`enhancement`がユニットに追随。`UI-AC-032`）
- スロットbuttonへのHTML5 drag&drop（空き枠はdrop先のみ・実行中は不可）
- キーボード代替の「移動」ボタンと移動モード（Escapeはdocumentレベルで捕捉、`aria-live`通知。`UI-NFR-001`維持）
- Component test `UI-CT-050`〜`052`、E2E `UI-E2E-011`／`012`

### 完了条件

- `UI-AC-032`と`UI-CT-050`〜`052`、`UI-E2E-011`／`012`を満たす。
- 陣営を跨ぐdrop・自分自身へのdrop・disabled中のdragが編成を変更しない。
- 既存のクリック→選択ダイアログ、focus復帰（`UI-CT-004`）、ステータスプレビュー（`UI-AC-027`）が退行しない。

## 14. Issue分割案

| 順位 | Task ID         | Issue  | Milestone | 内容                                             | 依存                                       |
| ---: | --------------- | ------ | --------- | ------------------------------------------------ | ------------------------------------------ |
|    1 | `M45-API-001`   | [#90]  | M4.5      | Catalog一覧Application query・選択可否projection | M1 Catalog                                 |
|    2 | `M45-API-002`   | [#91]  | M4.5      | 一覧GET DTO・Schema・OpenAPI・ETag               | M45-API-001                                |
|    3 | `M45-API-003`   | [#92]  | M4.5      | Fastify CORS設定・contract test                  | M4 API                                     |
|    4 | `M45-INFRA-001` | [#105] | M4.5      | Production container・Cloud Run service基盤      | M45-API-002,003                            |
|    5 | `M45-INFRA-002` | [#106] | M4.5      | Cloud Run CI/CD・smoke test・rollback            | M45-INFRA-001                              |
|    6 | `M45-UI-001`    | [#93]  | M4.5      | UI workspace・CI・theme/AppShell                 | なし                                       |
|    7 | `M45-ARCH-001`  | [#116] | M4.5      | workspaceをapps/api・apps/ui構成へ再編           | M45-UI-001                                 |
|    8 | `M45-UI-002`    | [#94]  | M4.5      | 一覧API client・load state・filter               | M45-API-002,003, M45-UI-001, M45-ARCH-001  |
|    9 | `M45-UI-003`    | [#95]  | M4.5      | 編成editor・選択dialog・draft validation         | M45-UI-002                                 |
|   10 | `M45-UI-004`    | [#96]  | M4.5      | 戦闘API client・execution state・error表示       | M45-API-003, M45-UI-001, M45-ARCH-001      |
|   11 | `M45-UI-005`    | [#97]  | M4.5      | outcome・summary・event/transition/JSON          | M45-UI-004                                 |
|   12 | `M45-UI-006`    | [#98]  | M4.5      | responsive・accessibility・visual/E2E            | M45-UI-003～005                            |
|   13 | `M45-UI-007`    | [#99]  | M4.5      | GitHub Pages deploy・CSP・live smoke             | M45-API-002,003, M45-INFRA-002, M45-UI-006 |
|   14 | `M5-UI-001`     | [#100] | M5        | 行動Queue・AP/EX・Cooldown・Charge表示           | M4.5, M5 Domain                            |
|   15 | `M6-UI-001`     | [#101] | M6        | PS因果・Memory発動・PP/EX表示                    | M5-UI-001, M6 Domain                       |
|   16 | `M7-009`        | [#182] | M7        | Heal集計・Effect/状態異常表示                    | M6-UI-001, M7 Domain                       |
|   17 | `DMG-010`       | [#191] | M8        | Shield・SubUnit・高度Damage内訳表示              | M7-009, M8 Domain                          |
|   18 | `REL-007`       | [#201] | M9        | 最大負荷・Security・運用hardening                | DMG-010, M9 release準備                    |

[#90]: https://github.com/komei0727/muvluvgg_battle_simulator/issues/90
[#91]: https://github.com/komei0727/muvluvgg_battle_simulator/issues/91
[#92]: https://github.com/komei0727/muvluvgg_battle_simulator/issues/92
[#93]: https://github.com/komei0727/muvluvgg_battle_simulator/issues/93
[#94]: https://github.com/komei0727/muvluvgg_battle_simulator/issues/94
[#95]: https://github.com/komei0727/muvluvgg_battle_simulator/issues/95
[#96]: https://github.com/komei0727/muvluvgg_battle_simulator/issues/96
[#97]: https://github.com/komei0727/muvluvgg_battle_simulator/issues/97
[#98]: https://github.com/komei0727/muvluvgg_battle_simulator/issues/98
[#99]: https://github.com/komei0727/muvluvgg_battle_simulator/issues/99
[#100]: https://github.com/komei0727/muvluvgg_battle_simulator/issues/100
[#101]: https://github.com/komei0727/muvluvgg_battle_simulator/issues/101
[#182]: https://github.com/komei0727/muvluvgg_battle_simulator/issues/182
[#191]: https://github.com/komei0727/muvluvgg_battle_simulator/issues/191
[#201]: https://github.com/komei0727/muvluvgg_battle_simulator/issues/201
[#105]: https://github.com/komei0727/muvluvgg_battle_simulator/issues/105
[#106]: https://github.com/komei0727/muvluvgg_battle_simulator/issues/106
[#116]: https://github.com/komei0727/muvluvgg_battle_simulator/issues/116

1つのPRでscaffold、CORS、全画面、deployを同時に完成させない。pure mapping/testをcomponentより先に固定し、各PRをレビュー可能な大きさにする。

## 15. AI実装時の指示

実装AIは各Taskで次を行う。

1. 本READMEの正本優先順位を確認する。
2. Taskに対応する `UI-AC`、`UI-ARCH`、`UI-API`、`UI-CMP`、`UI-NFR`、`UI-TEST`を列挙する。
3. 対象外機能を実装しない。
4. API/Catalog値をID文字列から推測しない。
5. 先にpure test、次にcomponent、最後にE2Eを追加する。
6. 採用HTMLモックを視覚比較に使うが、固定resultとmonolithic scriptをコピーしない。
7. 未知値fallbackと画像fallbackを維持する。
8. PR本文に満たしたTest IDと未完了項目を記載する。

## 16. UI初期リリース完了条件

- M4.5の `M45-API-001`～`003`、`M45-INFRA-001`～`002`、`M45-UI-001`、`M45-ARCH-001`、`M45-UI-002`～`007`が完了している。
- [01_UI要求・画面設計.md](./01_UI要求・画面設計.md) の `UI-AC-001`～`017`を満たす。
- GitHub PagesからM4 APIの最小戦闘を実行できる。
- HTMLモックと意図しない主要レイアウト差異がない。
- Workload Identity Federation、Cloud Run deploy、Pages deploy、CORS、rollback手順がある。
- M5以降のevent追加で壊れないunknown fallbackがtestされている。
