...
- どの状態異常かは`EffectStateResponse.statusKind`（M7-009でAPIへ追加した任意プロパティ、[10_API設計.md](../ddd/10_API設計.md)「EffectStateResponse」）を正本とし、`effectKindKey`（現状は効果アクション定義ID）の命名規則を解析しない。
