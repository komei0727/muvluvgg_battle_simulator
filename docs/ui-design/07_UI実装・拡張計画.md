...
- 「ユニット状態」タブが`finalState.units[].shields`（プール合計）と`subUnits[]`（インスタンス単位の耐久）を表示する。`effects`と同じ後方互換規約で、契約より前のfixtureでは「なし」と断定せず不明として扱う。シールド（DMG-004）とサブユニット（DMG-005）は別々の契約で追加されたため片方だけを持つレスポンスが存在しうる。まとめて1つの不明メッセージにすると欠落側が黙って消えるので、それぞれ独立に「不明」／「なし」／実値を出す（PR #301レビュー[P2]）。
- `CONTINUOUS_DAMAGE_APPLIED`の内訳にはサブユニット吸収（`subUnitAbsorbed`）も含める。R-SUB-01第1項どおり`FIXED`継続ダメージはサブユニットへも吸収されるため、この項が無いと`calculatedDamage`と`hitPointDamage`の差を説明できない。Domain payloadが当該量を発行していなかったため、`ContinuousDamageApplied`へ追加した（PR #301レビュー[P1]、[08\_ドメインイベント.md](../ddd/08_ドメインイベント.md)「ContinuousDamageApplied payload」）。公開ログのJSON schemaは`DamageApplied.subUnitAbsorbed`と同じく既に任意プロパティとして定義済みのため、wire契約の変更は無い。
