...
`pnpm-workspace.yaml`へ `ui` をworkspace packageとして追加する。ルートの既存backend scriptsを壊さず、必要に応じて `ui:*` scriptsをルートから委譲する。
...
- UI側にwire DTO型を置く。
- 初期実装ではbackendの `src/application/http-contract.ts` をブラウザから直接importしない。backend package boundaryとビルド対象を分離する。
...
- `UI-ARCH-001`: `ui/dist`を静的HTTP serverから配信して全主要機能が起動する。
