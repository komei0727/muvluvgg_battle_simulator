...
├── 実行状態管理、エラー正規化・表示案内
shared/api
├── 全featureが使うwire型（api-contract.ts）、HTTP送信（api-client.ts）、
│   腐敗防止層（response-validator.ts）
entities
├── feature横断の共有語彙（BattleMode・RosterEntry・BattleDraft/Side/スロット
│   関連型・EXERCISE_TURN_LIMIT・UiViolation。REF-055）
features/exercise
...
- `entities`は複数featureが共有する語彙（型と、型に密結合な導出関数）だけを持つ。
  値の初期化・業務ルール・UIロジックは持たず、各featureがそれを実装する
  （FSD本来の「エンティティごとにui/model/apiを持つスライス」までは作らない、
  最小構成の置き場。REF-055）。`entities`はどのfeatureへも依存しない。
  `shared/api`はwire契約が`entities`の語彙（`LogLevel`等）を埋め込むために
  `entities`へ依存してよいが、`entities`は`shared`へ依存しない。両者とも
  `features/*`のファイルには依存しない——feature固有のロジック（`entities`の
  型を使った初期化・検証・組み立て）は各featureに残る。
