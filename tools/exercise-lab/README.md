...
次は `lab gear-sensitivity` と同じである。

- 固定するもの（ユニット・配置・メモリー・敵・レベル・学園レベル）と、**基点編成を変えたら
  結果が無効になる**こと（「何を固定し、何を動かすか」参照）
- 探索対象を5ステータスに限り、**HP・防御は動かさない**こと——スコアは与ダメージで決まり、
  HP・防御は生存を通してしか効かないため、敗北しない編成では1手の限界効用がほぼ0になる
- 最大化する量（期待日次ベスト＋保証値。「何を最大化しているか」参照）
- **共通乱数法の同期が完全ではない**こと——ギア変更は乱数の消費順を変えるので、分散は下がるが
  独立seedと完全同期の中間である（「共通乱数法は完全には効かない」参照）
- **なぜ動いたかは出せない**こと——評価APIは数値しか返さない。到達した配分で単発実行を1回回し、
  UIの効果トレースで当て先を確認する（Phase A の観測は、そのうち「誰へ付いたか」だけを機械が
  読める形に絞ったものである）
...
数万試行を1回で走り切れないことがある。評価と観測のたびに `state.json` を書き出すので、
`--resume` で続きから回せる。**中断せずに走らせた軌跡と一致する** ——同じ配分を投げ直さず、
best-so-far 曲線の横軸（消費試行数）まで揃う。

```bash
uv run lab gear-plan configs/formation.yaml --seed abc123 \
  --player-data local_storage/player-data.json --budget 60000 \
  --out reports/gear-plan/ --resume
```

戻すのは2つである。

- **評価済みスコア** — 一括評価の乱数は送信seedと位相で決まるので、スコアを戻せば乱数の位置も
  戻る。枠別の与ダメージも一緒に戻す（篩いの指標そのものであり、落とすと再開後の順位が総スコアで
  の代用に変わる）。
- **観測したレジーム署名** — 単発実行は seed を受け取らないため、同じ配分を2度観測しても同じ
  署名が返る保証が無い。署名そのものを持ち越すしかない。

再開は**先頭から回し直す**。探索の手続きは決定的なので、中断した地点までは評価も観測もキャッシュ
から読み、1試行も投げずに同じ道を辿る。投げ直しになるのは、最後の書き出しから中断までに走った
1巡ぶんだけである。

`evaluations.csv` は追記なので、再開した実行では中断までに投げた試行の行も残る——投げ直しに
なった1巡ぶんは、同じ配分・同じ位相の行が二重に並ぶ。分析するときは重複を畳む。

`--seed`・基点編成・探索設定のいずれかが保存時と違えば、**読まずに落ちる**。別の乱数列で測った
値を混ぜると、報告した順位がどちらの範囲のものか分からなくなる。なお再開した実行では校正の相手
（基点）も保存済みの状態から読めてしまうため、**所要時間の見積りは出せない**。
...
`lab optimize` はギアを `--player-data` から固定して**編成**を探し、`lab gear-plan` は編成
（メモリーを含む）を固定して**ギア**を探す。探索変数が相補的なので、**片方だけでは同時最適に
届かない**。2つのツールにまたがる座標降下として往復させる。

1. `lab optimize` を回し、上位の編成を1つ選んで編成定義YAML（`configs/formation.yaml`）へ写す。
2. その編成を基点に `lab gear-plan --player-data local_storage/player-data.json` を回す。
3. 書き戻された `reports/gear-plan/player-data.json` を次の `lab optimize --player-data` へ渡す。
   編成探索が新しいギアを前提に編成を選び直す。
4. 選ばれた編成が前回と違えば 2 へ戻る。変わらなくなったら止める。

**基点編成を変えたら前回の理論値配分は無効になる。** とくにメモリーは順位を決める量を直接動かす
ので、1往復前の配分を流用してはいけない——3 の書き戻しは、2 でその編成に対して探した配分だけを
含む。往復ごとに `--out` を分ける（`state.json` は基点ごとの状態であり、基点が変われば
`--resume` は拒む）。

書き戻すのは**理論値**であり、手持ちの在庫ではない。手元で組める編成を見たい往復では、到達手順
（`steps.csv`）のグループ末端まで適用した配分へ直してから 3 を行う。
...
| `state.json`       | 中断・再開用の状態（評価済みスコアと観測した署名）。`--resume` が読み、評価と観測のたびに書き直す。            |
...
| `--yes` / `-y`     | 無効    | 所要時間の確認を省く。                         |
| `--resume`         | 無効    | `--out` の `state.json` から再開する。         |
...
分析する（候補どうしの比較は最も短い履歴へ揃える）。頻繁に欠けるなら `--screen-runs` /
`--confirm-runs` / `--final-runs` を下げる。
