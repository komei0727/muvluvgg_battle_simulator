...
**校正も予算の内である。** 校正で測る基点は反復1の篩いがキャッシュから読むので、最低予算は
**1反復ぶんちょうど**になる（校正を別に足さない）。届かない予算は必要な額を添えて
...
| ファイル                    | 内容                                                                                     |
| --------------------------- | ---------------------------------------------------------------------------------------- |
| `gear-plan.json`            | 到達した配分・署名の一覧・各段の履歴・再スタートの結果・ランク微調整と単価表・再現条件。 |
| `gear-plan-evaluations.csv` | 試行ごとの生値。全評価の正本。                                                           |

コンソールには基点の署名、Phase B の採用手、Phase C の再スタート結果、到達した配分
（基点からの増減つき）、到達した署名の一覧、Phase D の対象と単価表を出す。
...
各枝の到達点は**探索が使ったのと同じ乱数範囲**で並べている。その範囲へ過適合した枝が上に
来うるので、別の乱数範囲での確定は別途行う必要がある。反復をまたいで同じ位相を使い回して
いるのも同じ理由で、基点の評価を払い直さない代わりに範囲は固定されている。
